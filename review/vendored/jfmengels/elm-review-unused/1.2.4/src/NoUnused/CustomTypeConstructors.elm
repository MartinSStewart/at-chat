...
import NoUnused.Vendor.StringExtra as StringExtra
        ...
        node :: restOfNodes ->
            case Node.value node of
                ...
                Expression.Application ((Node _ (Expression.FunctionOrValue _ name)) :: restOfArgs) ->
                    if StringExtra.isCapitalized name then
...
registerUsedFunctionOrValue range moduleName name moduleContext =
    if not (StringExtra.isCapitalized name) then
        moduleContext
