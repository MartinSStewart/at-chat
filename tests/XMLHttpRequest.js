      ...
      send(data) {
        ...
        switch (this._url.protocol) {
          ...
          default:
            ...
            setTimeout(() => {
              let replaceAll = function(search, replacement, target) {
                  return target.replace(new RegExp(search, 'g'), replacement);
              };
              this.status = 200;

              let path = '..' + replaceAll('%20', ' ', this._url.path)
              switch (this.responseType)
              {
                case 'arraybuffer':
                    this.response = new Uint8Array(fs.readFileSync(path)).buffer;
                    break;
                case '':
                case 'text':
                default:
                    this.response = fs.readFileSync(path).toString();
                    break;
              }


              this._listeners.load[0](this);
            }, 10);
