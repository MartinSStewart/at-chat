...
// What an encrypted push carries. The sender wrote this line and encrypted it with the
// conversation's key, so opening it needs nothing but the key and a text decoder.
const notificationMessageText = "Hello @Someone Else, here's a message";
...
// back to when it can't open the one it was given (see Broadcast.encryptedDmText).
...
async function run() {
    ...
    // An encrypted message never reaches the server in the clear, so the sender writes the
    // notification too and the server forwards it. These check that a device holding the
    // conversation's key shows the line, and that every way of not holding it falls back
    // rather than showing the reader base64.
