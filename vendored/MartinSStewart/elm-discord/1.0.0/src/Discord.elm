...
type ScheduledEventId
    = ScheduledEventId Never


encodeId : Id idType -> JE.Value
encodeId id =
    JE.string (idToString id)
...
idToString id =
    idToUInt64 id |> UInt64.toString


idFromString : String -> Maybe (Id idType)
idFromString text =
...
idToUInt64 : Id idType -> UInt64
idToUInt64 (Id id) =
    id


idFromUInt64 : UInt64 -> Id idType
idFromUInt64 =
    Id
...
getChannelPayload authentication channelId =
    httpGet authentication decodeChannel [ "channels", idToString channelId ] []
...
deleteChannelPayload authentication channelId =
    httpDelete authentication decodeChannel [ "channels", idToString channelId ] [] (JE.string "")
...
getMessagesPayload authentication { channelId, limit, relativeTo } =
    httpGet
        ...
        [ "channels", idToString channelId, "messages" ]
        (Url.Builder.int "limit" limit
            :: (case relativeTo of
                    Around messageId ->
                        [ Url.Builder.string "around" (idToString messageId) ]

                    Before messageId ->
                        [ Url.Builder.string "before" (idToString messageId) ]

                    After messageId ->
                        [ Url.Builder.string "after" (idToString messageId) ]
...
getMessagePayload authentication { channelId, messageId } =
    httpGet
        authentication
        decodeMessage
        [ "channels", idToString channelId, "messages", idToString messageId ]
        []
...
createMessagePayload :
    ...
    -> HttpRequest Message
createMessagePayload authentication { channelId, content, replyTo, attachments, stickers } =
    httpPost
        ...
        (( "content", JE.string content )
            ...
            ++ (case stickers of
                    ...
                    _ ->
                        [ ( "sticker_ids", JE.list encodeId stickers ) ]
               )
            ++ (case attachments of
                    [] ->
                        []
...
createReactionPayload authentication { channelId, messageId, emoji } =
    httpPut
        ...
        (JD.succeed ())
        [ "channels"
        , idToString channelId
        , "messages"
        , idToString messageId
        , "reactions"
...
deleteAllReactionsPayload authentication { channelId, messageId } =
    httpDelete
        authentication
        (JD.succeed ())
        [ "channels", idToString channelId, "messages", idToString messageId, "reactions" ]
        []
        (JE.object [])
...
bulkDeleteMessagePayload authentication { channelId, firstMessage, secondMessage, restOfMessages } =
    httpDelete
        authentication
        (JD.succeed ())
        [ "channels", idToString channelId, "messages", "bulk-delete" ]
        []
        (JE.list JE.string (idToString firstMessage :: idToString secondMessage :: List.map idToString restOfMessages))
...
getChannelInvitesPayload authentication channelId =
    httpGet
        ...
        [ "channels", idToString channelId, "invites" ]
        []
        ...
        (JE.object
            (( "max_age"
             , case maxAge of
                ...
                :: (case targetUser of
                        Just targetUserId ->
                            [ ( "target_user", JE.string (idToString targetUserId) ) ]
...
deleteChannelPermissionPayload authentication { channelId, overwriteId } =
    httpDelete
        authentication
        (JD.list decodeInviteWithMetadata)
        [ "channels", idToString channelId, "permissions", idToString overwriteId ]
        []
        (JE.object [])
...
triggerTypingIndicatorPayload authentication channelId =
    httpPost
        ...
        authentication
        (JD.succeed ())
        [ "channels", idToString channelId, "typing" ]
        []
        (JE.object [])
...
getPinnedMessagesPayload authentication channelId =
    httpGet
        authentication
        (JD.list decodeMessage)
        [ "channels", idToString channelId, "pins" ]
        []
...
addPinnedChannelMessagePayload authentication { channelId, messageId } =
    httpPut
        authentication
        (JD.succeed ())
        [ "channels", idToString channelId, "pins", idToString messageId ]
        []
        (JE.object [])
...
listGuildEmojisPayload authentication guildId =
    httpGet
        authentication
        (JD.list decodeEmoji)
        [ "guilds", idToString guildId, "emojis" ]
        []
...
getGuildEmojisPayload authentication { guildId, emojiId } =
    httpGet
        authentication
        decodeEmoji
        [ "guilds", idToString guildId, "emojis", idToString emojiId ]
        []
...
getGuildPreviewPayload authentication guildId =
    httpGet
        authentication
        decodeGuildPreview
        [ "guilds", idToString guildId, "preview" ]
        []
...
deleteGuildPayload authentication guildId =
    httpDelete authentication (JD.succeed ()) [ "guilds", idToString guildId ] [] (JE.object [])
...
getGuildChannelsPayload authentication guildId =
    httpGet authentication (JD.list decodeChannel2) [ "guilds", idToString guildId, "channels" ] []
...
createGuildTextChannelPayload authentication config =
    httpPost
        ...
        authentication
        decodeChannel
        [ "guilds", idToString config.guildId, "channels" ]
        []
        (JE.object
...
getGuildMemberPayload authentication guildId userId =
    httpGet
        authentication
        decodeGuildMember
        [ "guilds", idToString guildId, "members", idToString userId ]
        []
...
listGuildMembersPayload authentication { guildId, limit, after } =
    httpGet
        ...
        [ "guilds", idToString guildId, "members" ]
        (Url.Builder.int "limit" limit
            :: (case after of
                    Included after_ ->
                        [ Url.Builder.string "after" (idToString after_) ]
...
listActiveThreadsPayload authentication guildId =
    httpGet
        authentication
        decodeActiveThreads
        [ "guilds", idToString guildId, "threads", "active" ]
        []
...
getPublicArchivedThreadsPayload authentication { channelId, before, limit } =
    ...
    httpGet
        ...
        [ "channels", idToString channelId, "threads", "archived", "public" ]
        queryParams
...
getPrivateArchivedThreadsPayload authentication { channelId, before, limit } =
    ...
    httpGet
        authentication
        decodeArchivedThreads
        [ "channels", idToString channelId, "threads", "archived", "private" ]
        queryParams
...
startThreadFromMessagePayload authentication { channelId, messageId, name, autoArchiveDuration, rateLimitPerUser } =
    httpPost
        ...
        authentication
        decodeChannel
        [ "channels", idToString channelId, "messages", idToString messageId, "threads" ]
        []
        (JE.object
            ([ ( "name", JE.string name )
             ]
...
searchThreadsPayload authentication { channelId, name, slop, tag, tagSetting, archived, sortBy, sortOrder, limit, offset, maxId, minId } =
    let
        ...
        tagParams =
            tag
                |> Maybe.map (List.map (\t -> Url.Builder.string "tag" (idToString t)))
                |> Maybe.withDefault []
        ...
        queryParams =
            tagParams
                ++ List.filterMap identity
                    ...
                    , Maybe.map (\id -> Url.Builder.string "max_id" (idToString id)) maxId
                    , Maybe.map (\id -> Url.Builder.string "min_id" (idToString id)) minId
                    ]
    in
    httpGet
        authentication
        decodeSearchThreadsResult
        [ "channels", idToString channelId, "threads", "search" ]
        queryParams
...
getUserPayload authentication userId =
    httpGet authentication decodeUser [ "users", idToString userId ] []
...
leaveGuildPayload authentication guildId =
    httpDelete
        authentication
        (JD.succeed ())
        [ "users", "@me", "guilds", idToString guildId ]
        []
        (JE.object [])
...
guildIconUrl { size, imageType } guildId iconHash =
    Url.Builder.crossOrigin
        discordCdnUrl
        [ "icons", idToString guildId, rawHash iconHash ++ imageExtensionPngJpgWebpGif imageType ]
        (imageSizeQuery size)
...
guildSplashUrl { size, imageType } guildId splashHash =
    Url.Builder.crossOrigin
        discordCdnUrl
        [ "splashes", idToString guildId, rawHash splashHash ++ imageExtensionPngJpgWebp imageType ]
        (imageSizeQuery size)
...
guildDiscoverySplashUrl { size, imageType } guildId discoverySplashHash =
    Url.Builder.crossOrigin
        discordCdnUrl
        [ "discovery-splashes", idToString guildId, rawHash discoverySplashHash ++ imageExtensionPngJpgWebp imageType ]
        (imageSizeQuery size)
...
guildBannerUrl { size, imageType } guildId splashHash =
    Url.Builder.crossOrigin
        discordCdnUrl
        [ "banners", idToString guildId, rawHash splashHash ++ imageExtensionPngJpgWebp imageType ]
        (imageSizeQuery size)
...
userAvatarUrl { size, imageType } guildId avatarHash =
    Url.Builder.crossOrigin
        discordCdnUrl
        [ "avatars", idToString guildId, rawHash avatarHash ++ imageExtensionPngJpgWebpGif imageType ]
        (imageSizeQuery size)
...
applicationIconUrl { size, imageType } applicationId applicationIconHash =
    Url.Builder.crossOrigin
        discordCdnUrl
        [ "app-icons", idToString applicationId, rawHash applicationIconHash ++ imageExtensionPngJpgWebp imageType ]
        (imageSizeQuery size)
...
applicationAssetUrl { size, imageType } applicationId applicationAssetHash =
    Url.Builder.crossOrigin
        discordCdnUrl
        [ "app-assets", idToString applicationId, rawHash applicationAssetHash ++ imageExtensionPngJpgWebp imageType ]
        (imageSizeQuery size)
...
achievementIconUrl { size, imageType } applicationId achievementId achievementIconHash =
    Url.Builder.crossOrigin
        discordCdnUrl
        [ "app-assets"
        , idToString applicationId
        , "achievements"
        , idToString achievementId
        , "icons"
...
teamIconUrl { size, imageType } teamId teamIconHash =
    Url.Builder.crossOrigin
        discordCdnUrl
        [ "team-icons", idToString teamId, rawHash teamIconHash ++ ".png" ]
        (imageSizeQuery size)
...
getStickerPacks : Task HttpError (List StickerPack)
getStickerPacks =
    toTask getStickerPacksPayload


getStickerPacksPayload : HttpRequest (List StickerPack)
getStickerPacksPayload =
    { method = "GET"
    , headers = []
    , url =
        Url.Builder.crossOrigin
...
decodeStickerPack : JD.Decoder StickerPack
decodeStickerPack =
    JD.succeed StickerPack
    ...
    JD.map4
        (\id type_ allow deny ->
            case type_ of
                ...
                1 ->
                    { id = RoleOrUserId_UserId (idToUInt64 id |> idFromUInt64)
                    , allow = allow
                    , deny = deny
                    }
                        |> JD.succeed
...
--memberCanReadChannel : Id UserId -> List Overwrite -> Bool
--memberCanReadChannel id overwrites =
--    memberHasChannelPermission .viewChannel id overwrites
    ...
    if member.userId == ownerId || List.any (\role -> role.permissions.administrator) memberRoles then
        ...
        let
            ...
            applyOverwrites relevantOverwrites value =
                if List.any (\overwrite -> permission overwrite.allow) relevantOverwrites then
                    True

                else if List.any (\overwrite -> permission overwrite.deny) relevantOverwrites then
                    False

                else
                    value
        in
        List.any (\role -> permission role.permissions) memberRoles
            |> applyOverwrites
...
decodeStickerItem : JD.Decoder StickerItem
decodeStickerItem =
    JD.succeed StickerItem
        |> JD.andMap (JD.field "id" decodeId)
        |> JD.andMap (JD.field "name" JD.string)
        |> JD.andMap (JD.field "format_type" decodeStickerFormatType)
...
decodeEmbed =
    JD.succeed Embed
        ...
        |> JD.andMap (decodeOptionalData "description" JD.string)
...
{-| Points at what a message relates to. A reply points at the message being replied to,
a thread starter message (`ThreadStarterMessage`) points at the message its thread was
started from, and a thread created message (`ThreadCreated`) only sets `channelId`, which
points at the thread that was created.
-}
type alias MessageReference =
    { messageId : OptionalData (Id MessageId)
    , channelId : OptionalData (Id ChannelId)
    , guildId : OptionalData (Id GuildId)
    }


decodeMessageReference : JD.Decoder MessageReference
decodeMessageReference =
    JD.succeed MessageReference
        |> JD.andMap (decodeOptionalData "message_id" decodeId)
        |> JD.andMap (decodeOptionalData "channel_id" decodeId)
...
decodeMessage =
    JD.succeed Message
        ...
        |> JD.andMap (JD.field "type" decodeMessageType)
        |> JD.andMap (decodeOptionalData "flags" decodeMessageFlags)
        |> JD.andMap
...
decodeAttachment =
    JD.succeed Attachment
        ...
        |> JD.andMap (decodeOptionalData "flags" decodeAttachmentFlags)
...
decodePrivateChannel =
    JD.succeed PrivateChannel
        |> JD.andMap (JD.field "type" decodeChannelType)
        |> JD.andMap (JD.field "recipient_ids" (JD.list decodeId))
        |> JD.andMap (JD.field "recipient_flags" JD.int)
        |> JD.andMap (JD.field "last_message_id" (JD.nullable decodeId))
...
urlEncodeEmoji emojiId =
    case emojiId of
        ...
        CustomEmoji emoji ->
            emoji.name ++ ":" ++ idToString emoji.id |> Url.percentEncode
...
decodeStickerFormatType : JD.Decoder StickerFormatType
decodeStickerFormatType =
    JD.andThen
        (\formatInt ->
            case formatInt of
                1 ->
...
decodeDispatchEvent eventName =
    case eventName of
        ...
        "TYPING_START" ->
            JD.field "d" decodeTypingStart |> JD.map DispatchBot_TypingStart

        _ ->
            JD.fail <| "Invalid event name: " ++ eventName


type alias TypingStart =
    { guildId : OptionalData (Id GuildId)
    , channelId : Id ChannelId
    , userId : Id UserId
    , timestamp : Time.Posix
...
decodeTypingStart : JD.Decoder TypingStart
decodeTypingStart =
    JD.succeed TypingStart
        |> JD.andMap (decodeOptionalData "guild_id" decodeId)
        |> JD.andMap (JD.field "channel_id" decodeId)
        |> JD.andMap (JD.field "user_id" decodeId)
...
type alias GuildScheduledEventUserData =
    { guildId : Id GuildId
    , scheduledEventId : Id ScheduledEventId
    , userId : Id UserId
    }


decodeGuildScheduledEventUserData : JD.Decoder GuildScheduledEventUserData
decodeGuildScheduledEventUserData =
    JD.succeed GuildScheduledEventUserData
        |> JD.andMap (JD.field "guild_id" decodeId)
        |> JD.andMap (JD.field "guild_scheduled_event_id" decodeId)
        |> JD.andMap (JD.field "user_id" decodeId)
...
decodeDispatchUserEvent eventName =
    case eventName of
        ...
        "GUILD_UPDATE" ->
            JD.field "d" decodeGuildUpdate |> JD.map DispatchUser_GuildUpdate

        "GUILD_DELETE" ->
            JD.field "d" decodeGuildDelete |> JD.map DispatchUser_GuildDelete

        "GUILD_APPLIED_BOOSTS_UPDATE" ->
            JD.succeed DispatchUser_GuildAppliedBoostsUpdate
        ...
        "GUILD_EMOJIS_UPDATE" ->
            JD.field "d" decodeGuildEmojisUpdate |> JD.map DispatchUser_GuildEmojisUpdate
        ...
        "GUILD_SCHEDULED_EVENT_USER_ADD" ->
            JD.field "d" decodeGuildScheduledEventUserData
                |> JD.map DispatchUser_GuildScheduledEventUserAdd

        "GUILD_SCHEDULED_EVENT_USER_REMOVE" ->
            JD.field "d" decodeGuildScheduledEventUserData
                |> JD.map DispatchUser_GuildScheduledEventUserRemove
...
decodeMessageFlags : JD.Decoder MessageFlags
decodeMessageFlags =
    JD.map
        (\bits ->
            let
            ...
            in
            { crossposted = getBit 0
            , isCrosspost = getBit 1
            , suppressEmbeds = getBit 2
            , sourceMessageDeleted = getBit 3
            , urgent = getBit 4
            , hasThread = getBit 5
            , ephemeral = getBit 6
            , loading = getBit 7
            , failedToMentionSomeRolesInThread = getBit 8
            , guildFeedHidden = getBit 9
            , shouldShowLinkNotDiscordWarning = getBit 10
            , suppressNotifications = getBit 12
            , isVoiceMessage = getBit 13
            , hasSnapshot = getBit 14
            , isComponentsV2 = getBit 15
            , sentBySocialLayerIntegration = getBit 16
            }
        )
        JD.int
...
decodeAttachmentFlags : JD.Decoder AttachmentFlags
decodeAttachmentFlags =
    JD.map
        (\bits ->
            ...
            in
            { isClip = getBit 0
            , isThumbnail = getBit 1
            , isRemix = getBit 2
            , isSpoiler = getBit 3
            , containsExplicitMedia = getBit 4
            , isAnimated = getBit 5
            , containsGoreContent = getBit 6
            , containsSelfHarmContent = getBit 7
            }
        )
        JD.int
...
type OpDispatchUserEvent
    ...
    | DispatchUser_GuildUpdate GuildUpdate
...
decodeContentInventoryInboxStale : JD.Decoder ContentInventoryInboxStale
decodeContentInventoryInboxStale =
    JD.succeed (\refreshAfter -> ContentInventoryInboxStale (Duration.milliseconds refreshAfter))
        |> JD.andMap (JD.field "refresh_after_ms" JD.float)
...
decodeReactionAdd =
    JD.succeed ReactionAdd
        |> JD.andMap (JD.field "user_id" decodeId)
        |> JD.andMap (JD.field "channel_id" decodeId)
        |> JD.andMap (JD.field "message_id" decodeId)
        |> JD.andMap (decodeOptionalData "guild_id" decodeId)
        |> JD.andMap (decodeOptionalData "member" decodeGuildMember)
        |> JD.andMap (JD.field "emoji" decodeEmoji)
        |> JD.andMap (decodeOptionalData "message_author_id" decodeId)
...
decodeGuildRoleUpdate =
    JD.succeed GuildRoleUpdate
        |> JD.andMap (JD.field "guild_id" decodeId)
        |> JD.andMap (JD.field "role" decodeRole)


{-| <https://docs.discord.food/gateway/gateway-events#guild-emojis-update>

`emojis` is the guild's full emoji list after the change, not just the emojis
that were added or removed.

-}
type alias GuildEmojisUpdate =
    { guildId : Id GuildId
    , emojis : List EmojiData
    }


decodeGuildEmojisUpdate : JD.Decoder GuildEmojisUpdate
decodeGuildEmojisUpdate =
    JD.succeed GuildEmojisUpdate
        |> JD.andMap (JD.field "guild_id" decodeId)
        |> JD.andMap (JD.field "emojis" (JD.list decodeEmoji))


{-| <https://docs.discord.food/gateway/gateway-events#guild-update>

Sent when a guild's settings change. The payload describes the guild as it looks
after the change and has the same shape as the `properties` of a GUILD\_CREATE,
except that the roles are inlined rather than sent separately. Only the fields
we keep track of are decoded.

-}
type alias GuildUpdate =
    { id : Id GuildId
    , name : String
    , icon : Maybe (ImageHash IconHash)
    , roles : List Role
...
decodeGuildUpdate : JD.Decoder GuildUpdate
decodeGuildUpdate =
    JD.succeed GuildUpdate
        |> JD.andMap (JD.field "id" decodeId)
        |> JD.andMap (JD.field "name" JD.string)
        |> JD.andMap (JD.field "icon" (JD.nullable decodeHash))
        |> JD.andMap (JD.field "roles" (JD.list decodeRole))
...
{-| <https://docs.discord.food/gateway/gateway-events#guild-delete>

Sent when the user leaves or is removed from a guild, or when the guild is deleted.
It's also sent when the guild becomes unavailable due to a Discord outage, in which
case `isUnavailable` is True and a GUILD\_CREATE follows once the outage is over.

-}
type alias GuildDelete =
    { id : Id GuildId
    , isUnavailable : Bool
    }


decodeGuildDelete : JD.Decoder GuildDelete
decodeGuildDelete =
    JD.succeed GuildDelete
        |> JD.andMap (JD.field "id" decodeId)
        |> JD.andMap (JD.oneOf [ JD.field "unavailable" JD.bool, JD.succeed False ])
...
encodeRequestGuildMembers guildIds nonce2 =
    JE.object
        [ ( "op", JE.int 8 )
        , ( "d"
          , [ ( "guild_id", JE.list encodeId guildIds )
            , ( "query", JE.string "" )
            , ( "limit", JE.int 1000 )
            ]
...
type UserOutMsg connection
    ...
    | UserOutMsg_GuildUpdate GuildUpdate
    | UserOutMsg_LeftOrDeletedGuild (Id GuildId)
    | UserOutMsg_GuildRoleUpdate GuildRoleUpdate
    | UserOutMsg_GuildEmojisUpdate GuildEmojisUpdate
    | UserOutMsg_ChannelPinsUpdate ChannelPinsUpdate
    | UserOutMsg_UserUpdate User
    | UserOutMsg_GuildScheduledEventUserAdd GuildScheduledEventUserData
    | UserOutMsg_GuildScheduledEventUserRemove GuildScheduledEventUserData
            ...
            case data of
                ...
                OpDispatch sequenceCounter opDispatchEvent ->
                    (case opDispatchEvent of
                        ...
                        DispatchUser_GuildUpdate guildUpdate ->
                            ( model, [ UserOutMsg_GuildUpdate guildUpdate ] )

                        DispatchUser_GuildDelete guildDelete ->
                            if guildDelete.isUnavailable then
                                -- The guild is only gone for the duration of a Discord outage and
                                -- we get a GUILD_CREATE again once it's over.
                                ( model, [] )

                            else
                                ( model, [ UserOutMsg_LeftOrDeletedGuild guildDelete.id ] )
                        ...
                        DispatchUser_GuildEmojisUpdate guildEmojisUpdate ->
                            ( model, [ UserOutMsg_GuildEmojisUpdate guildEmojisUpdate ] )
...
uploadAttachmentsPayload auth channelId attachments =
    httpPost
        ...
        (userToken auth)
        decodeUploadAttachmentResponse
        [ "channels", idToString channelId, "attachments" ]
        []
...
createPrivateChannelPayload auth recipients =
    httpPost
        ...
        [ "users", "@me", "channels" ]
        []
        (JE.object [ ( "recipients", JE.list encodeId recipients ) ])
