module Evergreen.V344.LocalState exposing (..)

import Array
import Date
import Effect.Http
import Effect.Lamdera
import Effect.Time
import Effect.Websocket
import Evergreen.V344.Call
import Evergreen.V344.ChannelDescription
import Evergreen.V344.ChannelName
import Evergreen.V344.Cloudflare
import Evergreen.V344.Discord
import Evergreen.V344.DiscordUserData
import Evergreen.V344.DmChannel
import Evergreen.V344.DmChannelId
import Evergreen.V344.Drawing
import Evergreen.V344.FileStatus
import Evergreen.V344.Game
import Evergreen.V344.GuildName
import Evergreen.V344.Id
import Evergreen.V344.IdArray
import Evergreen.V344.Log
import Evergreen.V344.MembersAndOwner
import Evergreen.V344.Message
import Evergreen.V344.MessageArray
import Evergreen.V344.NonemptyDict
import Evergreen.V344.OneToOne
import Evergreen.V344.Pagination
import Evergreen.V344.Postmark
import Evergreen.V344.SecretId
import Evergreen.V344.SessionIdHash
import Evergreen.V344.Slack
import Evergreen.V344.TextEditor
import Evergreen.V344.Thread
import Evergreen.V344.ToBackendLog
import Evergreen.V344.User
import Evergreen.V344.UserSession
import Evergreen.V344.VisibleMessages
import SeqDict
import SeqSet


type PrivateVapidKey
    = PrivateVapidKey String


type alias AdminData_DmChannel =
    { messageCount : Int
    , threadCount : Int
    }


type alias AdminData_DiscordDmChannel =
    { members :
        Evergreen.V344.NonemptyDict.NonemptyDict
            (Evergreen.V344.Discord.Id Evergreen.V344.Discord.UserId)
            { messagesSent : Int
            }
    , messageCount : Int
    , firstMessage : Maybe (Evergreen.V344.Message.Message Evergreen.V344.Id.ChannelMessageId (Evergreen.V344.Discord.Id Evergreen.V344.Discord.UserId))
    }


type DiscordUserData_ForAdmin
    = BasicData_ForAdmin
        { user : Evergreen.V344.Discord.PartialUser
        , icon : Maybe Evergreen.V344.FileStatus.FileHash
        }
    | FullData_ForAdmin
        { user : Evergreen.V344.Discord.User
        , linkedTo : Evergreen.V344.Id.Id Evergreen.V344.Id.UserId
        , icon : Maybe Evergreen.V344.FileStatus.FileHash
        , linkedAt : Effect.Time.Posix
        , isLoadingData : Evergreen.V344.DiscordUserData.DiscordUserLoadingData
        }
    | NeedsAuthAgain_ForAdmin
        { user : Evergreen.V344.Discord.User
        , linkedTo : Evergreen.V344.Id.Id Evergreen.V344.Id.UserId
        , icon : Maybe Evergreen.V344.FileStatus.FileHash
        , linkedAt : Effect.Time.Posix
        }


type alias AdminData_DiscordChannel =
    { name : Evergreen.V344.ChannelName.ChannelName
    , messageCount : Int
    , threadCount : Int
    , firstMessage : Maybe (Evergreen.V344.Message.Message Evergreen.V344.Id.ChannelMessageId (Evergreen.V344.Discord.Id Evergreen.V344.Discord.UserId))
    }


type alias DiscordRole =
    { name : String
    , description : Maybe String
    , permissions : Evergreen.V344.Discord.Permissions
    }


type alias AdminData_DiscordGuild =
    { name : Evergreen.V344.GuildName.GuildName
    , channels : SeqDict.SeqDict (Evergreen.V344.Discord.Id Evergreen.V344.Discord.ChannelId) AdminData_DiscordChannel
    , membersAndOwner :
        Evergreen.V344.MembersAndOwner.MembersAndOwner
            (Evergreen.V344.Discord.Id Evergreen.V344.Discord.UserId)
            { joinedAt : Maybe Effect.Time.Posix
            , roles : SeqSet.SeqSet (Evergreen.V344.Discord.Id Evergreen.V344.Discord.RoleId)
            }
    , roles : SeqDict.SeqDict (Evergreen.V344.Discord.Id Evergreen.V344.Discord.RoleId) DiscordRole
    }


type alias AdminData_GuildChannel =
    { name : Evergreen.V344.ChannelName.ChannelName
    , messageCount : Int
    }


type alias AdminData_Guild =
    { name : Evergreen.V344.GuildName.GuildName
    , channels : SeqDict.SeqDict (Evergreen.V344.Id.Id Evergreen.V344.Id.ChannelId) AdminData_GuildChannel
    , memberCount : Int
    , owner : Evergreen.V344.Id.Id Evergreen.V344.Id.UserId
    }


type alias AdminData_DeletedGuild =
    { name : Evergreen.V344.GuildName.GuildName
    , owner : Evergreen.V344.Id.Id Evergreen.V344.Id.UserId
    , memberCount : Int
    , deletedAt : Effect.Time.Posix
    }


type LoadingDiscordChannelStep messages
    = LoadingDiscordChannelMessages
    | LoadingDiscordChannelMessagesFailed Evergreen.V344.Discord.HttpError
    | LoadingDiscordChannelAttachments Effect.Time.Posix messages


type LoadingDiscordChannel messages
    = LoadingDiscordDmChannel Effect.Time.Posix (Evergreen.V344.Discord.Id Evergreen.V344.Discord.PrivateChannelId) (LoadingDiscordChannelStep messages)
    | LoadingDiscordGuildChannel Effect.Time.Posix (Evergreen.V344.Discord.Id Evergreen.V344.Discord.GuildId) (Evergreen.V344.Discord.Id Evergreen.V344.Discord.ChannelId) (LoadingDiscordChannelStep messages)


type alias LogWithTime =
    { time : Effect.Time.Posix
    , log : Evergreen.V344.Log.Log
    , isHidden : Bool
    }


type LastRequest
    = NoRequestsMade
    | LastRequest Effect.Time.Posix


type CallStatus
    = NotInCall
    | ConnectingToCall Evergreen.V344.Call.CallId
    | ConnectedToCall
        Evergreen.V344.Call.CallId
        { sessionId : Evergreen.V344.Cloudflare.RealtimeSessionId
        , trackNames : List Evergreen.V344.Cloudflare.TrackName
        , pullTracksReady : Bool
        }


type alias ConnectionData =
    { lastRequest : LastRequest
    , call : CallStatus
    , remoteCallData : Evergreen.V344.Call.RemoteCallData
    , currentlyViewing : Evergreen.V344.UserSession.Viewing
    }


type WebsocketClosedEvent
    = WebsocketClosed_CloseAndReopenForUser (Evergreen.V344.Discord.Id Evergreen.V344.Discord.UserId) Effect.Time.Posix
    | WebsocketClosed_UnlinkDiscordUser (Evergreen.V344.Discord.Id Evergreen.V344.Discord.UserId) Effect.Time.Posix
    | WebsocketClosed_ClosedByBackendForUser (Evergreen.V344.Discord.Id Evergreen.V344.Discord.UserId) Effect.Time.Posix
    | WebsocketClosed_ListenCloseEvent (Evergreen.V344.Discord.Id Evergreen.V344.Discord.UserId) Effect.Websocket.CloseEventCode String Effect.Time.Posix


type WordSpellingGameStatus
    = WordSpellingGameStatus_NotLoaded
    | WordSpellingGameStatus_Loading
    | WordSpellingGameStatus_Error Effect.Http.Error
    | WordSpellingGameStatus_Loaded


type alias Archived =
    { archivedAt : Effect.Time.Posix
    , archivedBy : Evergreen.V344.Id.Id Evergreen.V344.Id.UserId
    }


type alias FrontendChannel =
    { createdAt : Effect.Time.Posix
    , createdBy : Evergreen.V344.Id.Id Evergreen.V344.Id.UserId
    , name : Evergreen.V344.ChannelName.ChannelName
    , description : Evergreen.V344.ChannelDescription.ChannelDescription
    , messages : Evergreen.V344.MessageArray.MessageArray Evergreen.V344.Id.ChannelMessageId (Evergreen.V344.Message.Message Evergreen.V344.Id.ChannelMessageId (Evergreen.V344.Id.Id Evergreen.V344.Id.UserId))
    , visibleMessages : Evergreen.V344.VisibleMessages.VisibleMessages Evergreen.V344.Id.ChannelMessageId
    , isArchived : Maybe Archived
    , lastTypedAt : SeqDict.SeqDict (Evergreen.V344.Id.Id Evergreen.V344.Id.UserId) (Evergreen.V344.Thread.LastTypedAt Evergreen.V344.Id.ChannelMessageId)
    , threads : SeqDict.SeqDict (Evergreen.V344.Id.Id Evergreen.V344.Id.ChannelMessageId) Evergreen.V344.Thread.FrontendThread
    , dateDividerDrawings : SeqDict.SeqDict Date.Date (Evergreen.V344.Drawing.Drawing (Evergreen.V344.Id.Id Evergreen.V344.Id.UserId))
    , games : SeqDict.SeqDict (Evergreen.V344.Id.Id Evergreen.V344.Id.ChannelMessageId) Evergreen.V344.Game.MatchData
    }


type alias FrontendGuild =
    { createdAt : Effect.Time.Posix
    , createdBy : Evergreen.V344.Id.Id Evergreen.V344.Id.UserId
    , name : Evergreen.V344.GuildName.GuildName
    , icon : Maybe Evergreen.V344.FileStatus.FileHash
    , channels : SeqDict.SeqDict (Evergreen.V344.Id.Id Evergreen.V344.Id.ChannelId) FrontendChannel
    , membersAndOwner :
        Evergreen.V344.MembersAndOwner.MembersAndOwner
            (Evergreen.V344.Id.Id Evergreen.V344.Id.UserId)
            { joinedAt : Effect.Time.Posix
            }
    , invites :
        SeqDict.SeqDict
            (Evergreen.V344.SecretId.SecretId Evergreen.V344.Id.InviteLinkId)
            { createdAt : Effect.Time.Posix
            , createdBy : Evergreen.V344.Id.Id Evergreen.V344.Id.UserId
            }
    }


type alias DiscordFrontendChannel =
    { name : Evergreen.V344.ChannelName.ChannelName
    , description : Evergreen.V344.ChannelDescription.ChannelDescription
    , messages : Evergreen.V344.MessageArray.MessageArray Evergreen.V344.Id.ChannelMessageId (Evergreen.V344.Message.Message Evergreen.V344.Id.ChannelMessageId (Evergreen.V344.Discord.Id Evergreen.V344.Discord.UserId))
    , visibleMessages : Evergreen.V344.VisibleMessages.VisibleMessages Evergreen.V344.Id.ChannelMessageId
    , lastTypedAt : SeqDict.SeqDict (Evergreen.V344.Discord.Id Evergreen.V344.Discord.UserId) (Evergreen.V344.Thread.LastTypedAt Evergreen.V344.Id.ChannelMessageId)
    , threads : SeqDict.SeqDict (Evergreen.V344.Id.Id Evergreen.V344.Id.ChannelMessageId) Evergreen.V344.Thread.DiscordFrontendThread
    , dateDividerDrawings : SeqDict.SeqDict Date.Date (Evergreen.V344.Drawing.Drawing (Evergreen.V344.Discord.Id Evergreen.V344.Discord.UserId))
    , permissionOverwrites : List Evergreen.V344.Discord.Overwrite
    }


type alias DiscordFrontendGuild =
    { name : Evergreen.V344.GuildName.GuildName
    , icon : Maybe Evergreen.V344.FileStatus.FileHash
    , channels : SeqDict.SeqDict (Evergreen.V344.Discord.Id Evergreen.V344.Discord.ChannelId) DiscordFrontendChannel
    , membersAndOwner :
        Evergreen.V344.MembersAndOwner.MembersAndOwner
            (Evergreen.V344.Discord.Id Evergreen.V344.Discord.UserId)
            { joinedAt : Maybe Effect.Time.Posix
            , roles : SeqSet.SeqSet (Evergreen.V344.Discord.Id Evergreen.V344.Discord.RoleId)
            }
    , stickers : SeqSet.SeqSet (Evergreen.V344.Id.Id Evergreen.V344.Id.StickerId)
    , customEmojis : SeqSet.SeqSet (Evergreen.V344.Id.Id Evergreen.V344.Id.CustomEmojiId)
    , roles : SeqDict.SeqDict (Evergreen.V344.Discord.Id Evergreen.V344.Discord.RoleId) DiscordRole
    }


type JoinGuildError
    = AlreadyJoined
    | InviteIsInvalid


type ServerSecretStatus
    = NotBeingRegenerated (Maybe Effect.Time.Posix)
    | BeingRegenerated
    | RegenerationFailed Effect.Http.Error


type alias AdminData =
    { users : Evergreen.V344.NonemptyDict.NonemptyDict (Evergreen.V344.Id.Id Evergreen.V344.Id.UserId) Evergreen.V344.User.BackendUser
    , emailNotificationsEnabled : Bool
    , twoFactorAuthentication : SeqDict.SeqDict (Evergreen.V344.Id.Id Evergreen.V344.Id.UserId) Effect.Time.Posix
    , privateVapidKey : PrivateVapidKey
    , slackClientSecret : Maybe Evergreen.V344.Slack.ClientSecret
    , openRouterKey : Maybe String
    , cloudflareRealtimeApiToken : Maybe Evergreen.V344.Cloudflare.RealtimeApiToken
    , cloudflareRealtimeAppId : Maybe Evergreen.V344.Cloudflare.AppId
    , cloudflareAccountId : Maybe Evergreen.V344.Cloudflare.AccountId
    , cloudflareAnalyticsApiToken : Maybe Evergreen.V344.Cloudflare.AnalyticsApiToken
    , postmarkKey : Evergreen.V344.Postmark.ApiKey
    , dmChannels : SeqDict.SeqDict Evergreen.V344.DmChannelId.DmChannelId AdminData_DmChannel
    , discordDmChannels : SeqDict.SeqDict (Evergreen.V344.Discord.Id Evergreen.V344.Discord.PrivateChannelId) AdminData_DiscordDmChannel
    , discordUsers : SeqDict.SeqDict (Evergreen.V344.Discord.Id Evergreen.V344.Discord.UserId) DiscordUserData_ForAdmin
    , discordGuilds : SeqDict.SeqDict (Evergreen.V344.Discord.Id Evergreen.V344.Discord.GuildId) AdminData_DiscordGuild
    , guilds : SeqDict.SeqDict (Evergreen.V344.Id.Id Evergreen.V344.Id.GuildId) AdminData_Guild
    , deletedGuilds : SeqDict.SeqDict (Evergreen.V344.Id.Id Evergreen.V344.Id.GuildId) AdminData_DeletedGuild
    , loadingDiscordChannels : SeqDict.SeqDict (Evergreen.V344.Discord.Id Evergreen.V344.Discord.UserId) (LoadingDiscordChannel Int)
    , signupsEnabled : Bool
    , discordLinkingEnabled : Bool
    , logs : Evergreen.V344.Pagination.Pagination LogWithTime
    , connections : SeqDict.SeqDict Evergreen.V344.SessionIdHash.SessionIdHash (Evergreen.V344.NonemptyDict.NonemptyDict Effect.Lamdera.ClientId ConnectionData)
    , filesCount : Int
    , toBackendLogs : Array.Array Evergreen.V344.ToBackendLog.ToBackendLogData
    , vulnerabilityChecks : String
    , serverSecretRefreshedAt : ServerSecretStatus
    , websocketCloseEvents : Array.Array WebsocketClosedEvent
    , sessions : SeqDict.SeqDict Evergreen.V344.SessionIdHash.SessionIdHash Evergreen.V344.UserSession.UserSession
    , wordSpellingGameEnglish : WordSpellingGameStatus
    , wordSpellingGameSwedish : WordSpellingGameStatus
    }


type AdminStatus
    = IsAdmin AdminData
    | IsAdminButDataNotLoaded
    | IsNotAdmin


type alias LocalState =
    { adminData : AdminStatus
    , guilds : SeqDict.SeqDict (Evergreen.V344.Id.Id Evergreen.V344.Id.GuildId) FrontendGuild
    , discordGuilds : SeqDict.SeqDict (Evergreen.V344.Discord.Id Evergreen.V344.Discord.GuildId) DiscordFrontendGuild
    , dmChannels : SeqDict.SeqDict (Evergreen.V344.Id.Id Evergreen.V344.Id.UserId) Evergreen.V344.DmChannel.FrontendDmChannel
    , discordDmChannels : SeqDict.SeqDict (Evergreen.V344.Discord.Id Evergreen.V344.Discord.PrivateChannelId) Evergreen.V344.DmChannel.DiscordFrontendDmChannel
    , joinGuildError : Maybe JoinGuildError
    , localUser : Evergreen.V344.User.LocalUser
    , otherSessions : SeqDict.SeqDict Evergreen.V344.SessionIdHash.SessionIdHash Evergreen.V344.UserSession.FrontendUserSession
    , publicVapidKey : String
    , textEditor : Evergreen.V344.TextEditor.LocalState
    , calls : Evergreen.V344.Call.Local
    }


type ChannelStatus
    = ChannelActive
    | ChannelDeleted
        { deletedAt : Effect.Time.Posix
        , deletedBy : Evergreen.V344.Id.Id Evergreen.V344.Id.UserId
        }


type alias BackendChannel =
    { createdAt : Effect.Time.Posix
    , createdBy : Evergreen.V344.Id.Id Evergreen.V344.Id.UserId
    , name : Evergreen.V344.ChannelName.ChannelName
    , description : Evergreen.V344.ChannelDescription.ChannelDescription
    , messages : Evergreen.V344.IdArray.IdArray Evergreen.V344.Id.ChannelMessageId (Evergreen.V344.Message.Message Evergreen.V344.Id.ChannelMessageId (Evergreen.V344.Id.Id Evergreen.V344.Id.UserId))
    , status : ChannelStatus
    , lastTypedAt : SeqDict.SeqDict (Evergreen.V344.Id.Id Evergreen.V344.Id.UserId) (Evergreen.V344.Thread.LastTypedAt Evergreen.V344.Id.ChannelMessageId)
    , threads : SeqDict.SeqDict (Evergreen.V344.Id.Id Evergreen.V344.Id.ChannelMessageId) Evergreen.V344.Thread.BackendThread
    , dateDividerDrawings : SeqDict.SeqDict Date.Date (Evergreen.V344.Drawing.Drawing (Evergreen.V344.Id.Id Evergreen.V344.Id.UserId))
    , games : SeqDict.SeqDict (Evergreen.V344.Id.Id Evergreen.V344.Id.ChannelMessageId) Evergreen.V344.Game.BackendGameData
    }


type alias BackendGuild =
    { createdAt : Effect.Time.Posix
    , createdBy : Evergreen.V344.Id.Id Evergreen.V344.Id.UserId
    , name : Evergreen.V344.GuildName.GuildName
    , icon : Maybe Evergreen.V344.FileStatus.FileHash
    , channels : SeqDict.SeqDict (Evergreen.V344.Id.Id Evergreen.V344.Id.ChannelId) BackendChannel
    , membersAndOwner :
        Evergreen.V344.MembersAndOwner.MembersAndOwner
            (Evergreen.V344.Id.Id Evergreen.V344.Id.UserId)
            { joinedAt : Effect.Time.Posix
            }
    , invites :
        SeqDict.SeqDict
            (Evergreen.V344.SecretId.SecretId Evergreen.V344.Id.InviteLinkId)
            { createdAt : Effect.Time.Posix
            , createdBy : Evergreen.V344.Id.Id Evergreen.V344.Id.UserId
            }
    }


type alias DeletedBackendGuild =
    { guild : BackendGuild
    , deletedAt : Effect.Time.Posix
    }


type alias DiscordBackendChannel =
    { name : Evergreen.V344.ChannelName.ChannelName
    , description : Evergreen.V344.ChannelDescription.ChannelDescription
    , messages : Evergreen.V344.IdArray.IdArray Evergreen.V344.Id.ChannelMessageId (Evergreen.V344.Message.Message Evergreen.V344.Id.ChannelMessageId (Evergreen.V344.Discord.Id Evergreen.V344.Discord.UserId))
    , status : ChannelStatus
    , lastTypedAt : SeqDict.SeqDict (Evergreen.V344.Discord.Id Evergreen.V344.Discord.UserId) (Evergreen.V344.Thread.LastTypedAt Evergreen.V344.Id.ChannelMessageId)
    , linkedMessageIds : Evergreen.V344.OneToOne.OneToOne (Evergreen.V344.Discord.Id Evergreen.V344.Discord.MessageId) (Evergreen.V344.Id.Id Evergreen.V344.Id.ChannelMessageId)
    , threads : SeqDict.SeqDict (Evergreen.V344.Id.Id Evergreen.V344.Id.ChannelMessageId) Evergreen.V344.Thread.DiscordBackendThread
    , dateDividerDrawings : SeqDict.SeqDict Date.Date (Evergreen.V344.Drawing.Drawing (Evergreen.V344.Discord.Id Evergreen.V344.Discord.UserId))
    , permissionOverwrites : List Evergreen.V344.Discord.Overwrite
    }


type alias DiscordBackendGuild =
    { name : Evergreen.V344.GuildName.GuildName
    , icon : Maybe Evergreen.V344.FileStatus.FileHash
    , channels : SeqDict.SeqDict (Evergreen.V344.Discord.Id Evergreen.V344.Discord.ChannelId) DiscordBackendChannel
    , membersAndOwner :
        Evergreen.V344.MembersAndOwner.MembersAndOwner
            (Evergreen.V344.Discord.Id Evergreen.V344.Discord.UserId)
            { joinedAt : Maybe Effect.Time.Posix
            , roles : SeqSet.SeqSet (Evergreen.V344.Discord.Id Evergreen.V344.Discord.RoleId)
            }
    , stickers : SeqSet.SeqSet (Evergreen.V344.Id.Id Evergreen.V344.Id.StickerId)
    , customEmojis : SeqSet.SeqSet (Evergreen.V344.Id.Id Evergreen.V344.Id.CustomEmojiId)
    , roles : SeqDict.SeqDict (Evergreen.V344.Discord.Id Evergreen.V344.Discord.RoleId) DiscordRole
    }


type alias DiscordThreadReload =
    { threadId : Evergreen.V344.Discord.Id Evergreen.V344.Discord.ChannelId
    , messages : List Evergreen.V344.Discord.Message
    }


type alias DiscordChannelReload =
    { messages : List Evergreen.V344.Discord.Message
    , threads : List DiscordThreadReload
    }
