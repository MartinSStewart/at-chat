module Evergreen.V298.LocalState exposing (..)

import Array
import Date
import Effect.Http
import Effect.Lamdera
import Effect.Time
import Effect.Websocket
import Evergreen.V298.Call
import Evergreen.V298.ChannelDescription
import Evergreen.V298.ChannelName
import Evergreen.V298.Cloudflare
import Evergreen.V298.Discord
import Evergreen.V298.DiscordUserData
import Evergreen.V298.DmChannel
import Evergreen.V298.Drawing
import Evergreen.V298.FileStatus
import Evergreen.V298.GuildName
import Evergreen.V298.Id
import Evergreen.V298.IdArray
import Evergreen.V298.Log
import Evergreen.V298.MembersAndOwner
import Evergreen.V298.Message
import Evergreen.V298.NonemptyDict
import Evergreen.V298.OneToOne
import Evergreen.V298.Pagination
import Evergreen.V298.Postmark
import Evergreen.V298.SecretId
import Evergreen.V298.SessionIdHash
import Evergreen.V298.Slack
import Evergreen.V298.TextEditor
import Evergreen.V298.Thread
import Evergreen.V298.ToBackendLog
import Evergreen.V298.User
import Evergreen.V298.UserSession
import Evergreen.V298.VisibleMessages
import SeqDict
import SeqSet


type PrivateVapidKey
    = PrivateVapidKey String


type alias AdminData_DmChannel =
    { messageCount : Int
    , threadCount : Int
    }


type alias AdminData_DiscordDmChannel =
    { members :
        Evergreen.V298.NonemptyDict.NonemptyDict
            (Evergreen.V298.Discord.Id Evergreen.V298.Discord.UserId)
            { messagesSent : Int
            }
    , messageCount : Int
    , firstMessage : Maybe (Evergreen.V298.Message.Message Evergreen.V298.Id.ChannelMessageId (Evergreen.V298.Discord.Id Evergreen.V298.Discord.UserId))
    }


type DiscordUserData_ForAdmin
    = BasicData_ForAdmin
        { user : Evergreen.V298.Discord.PartialUser
        , icon : Maybe Evergreen.V298.FileStatus.FileHash
        }
    | FullData_ForAdmin
        { user : Evergreen.V298.Discord.User
        , linkedTo : Evergreen.V298.Id.Id Evergreen.V298.Id.UserId
        , icon : Maybe Evergreen.V298.FileStatus.FileHash
        , linkedAt : Effect.Time.Posix
        , isLoadingData : Evergreen.V298.DiscordUserData.DiscordUserLoadingData
        }
    | NeedsAuthAgain_ForAdmin
        { user : Evergreen.V298.Discord.User
        , linkedTo : Evergreen.V298.Id.Id Evergreen.V298.Id.UserId
        , icon : Maybe Evergreen.V298.FileStatus.FileHash
        , linkedAt : Effect.Time.Posix
        }


type alias AdminData_DiscordChannel =
    { name : Evergreen.V298.ChannelName.ChannelName
    , messageCount : Int
    , threadCount : Int
    , firstMessage : Maybe (Evergreen.V298.Message.Message Evergreen.V298.Id.ChannelMessageId (Evergreen.V298.Discord.Id Evergreen.V298.Discord.UserId))
    }


type alias AdminData_DiscordGuild =
    { name : Evergreen.V298.GuildName.GuildName
    , channels : SeqDict.SeqDict (Evergreen.V298.Discord.Id Evergreen.V298.Discord.ChannelId) AdminData_DiscordChannel
    , membersAndOwner :
        Evergreen.V298.MembersAndOwner.MembersAndOwner
            (Evergreen.V298.Discord.Id Evergreen.V298.Discord.UserId)
            { joinedAt : Maybe Effect.Time.Posix
            }
    }


type alias AdminData_GuildChannel =
    { name : Evergreen.V298.ChannelName.ChannelName
    , messageCount : Int
    }


type alias AdminData_Guild =
    { name : Evergreen.V298.GuildName.GuildName
    , channels : SeqDict.SeqDict (Evergreen.V298.Id.Id Evergreen.V298.Id.ChannelId) AdminData_GuildChannel
    , memberCount : Int
    , owner : Evergreen.V298.Id.Id Evergreen.V298.Id.UserId
    }


type alias AdminData_DeletedGuild =
    { name : Evergreen.V298.GuildName.GuildName
    , owner : Evergreen.V298.Id.Id Evergreen.V298.Id.UserId
    , memberCount : Int
    , deletedAt : Effect.Time.Posix
    }


type LoadingDiscordChannelStep messages
    = LoadingDiscordChannelMessages
    | LoadingDiscordChannelMessagesFailed Evergreen.V298.Discord.HttpError
    | LoadingDiscordChannelAttachments Effect.Time.Posix messages


type LoadingDiscordChannel messages
    = LoadingDiscordDmChannel Effect.Time.Posix (Evergreen.V298.Discord.Id Evergreen.V298.Discord.PrivateChannelId) (LoadingDiscordChannelStep messages)
    | LoadingDiscordGuildChannel Effect.Time.Posix (Evergreen.V298.Discord.Id Evergreen.V298.Discord.GuildId) (Evergreen.V298.Discord.Id Evergreen.V298.Discord.ChannelId) (LoadingDiscordChannelStep messages)


type alias LogWithTime =
    { time : Effect.Time.Posix
    , log : Evergreen.V298.Log.Log
    , isHidden : Bool
    }


type LastRequest
    = NoRequestsMade
    | LastRequest Effect.Time.Posix


type CallStatus
    = NotInCall
    | ConnectingToCall Evergreen.V298.Call.CallId
    | ConnectedToCall
        Evergreen.V298.Call.CallId
        { sessionId : Evergreen.V298.Cloudflare.RealtimeSessionId
        , trackNames : List Evergreen.V298.Cloudflare.TrackName
        , pullTracksReady : Bool
        }


type alias ConnectionData =
    { lastRequest : LastRequest
    , call : CallStatus
    , remoteCallData : Evergreen.V298.Call.RemoteCallData
    }


type WebsocketClosedEvent
    = WebsocketClosed_CloseAndReopenForUser (Evergreen.V298.Discord.Id Evergreen.V298.Discord.UserId) Effect.Time.Posix
    | WebsocketClosed_UnlinkDiscordUser (Evergreen.V298.Discord.Id Evergreen.V298.Discord.UserId) Effect.Time.Posix
    | WebsocketClosed_ClosedByBackendForUser (Evergreen.V298.Discord.Id Evergreen.V298.Discord.UserId) Effect.Time.Posix
    | WebsocketClosed_ListenCloseEvent (Evergreen.V298.Discord.Id Evergreen.V298.Discord.UserId) Effect.Websocket.CloseEventCode String Effect.Time.Posix


type alias Archived =
    { archivedAt : Effect.Time.Posix
    , archivedBy : Evergreen.V298.Id.Id Evergreen.V298.Id.UserId
    }


type alias FrontendChannel =
    { createdAt : Effect.Time.Posix
    , createdBy : Evergreen.V298.Id.Id Evergreen.V298.Id.UserId
    , name : Evergreen.V298.ChannelName.ChannelName
    , description : Evergreen.V298.ChannelDescription.ChannelDescription
    , messages : Evergreen.V298.IdArray.IdArray Evergreen.V298.Id.ChannelMessageId (Evergreen.V298.Message.MessageState Evergreen.V298.Id.ChannelMessageId (Evergreen.V298.Id.Id Evergreen.V298.Id.UserId))
    , visibleMessages : Evergreen.V298.VisibleMessages.VisibleMessages Evergreen.V298.Id.ChannelMessageId
    , isArchived : Maybe Archived
    , lastTypedAt : SeqDict.SeqDict (Evergreen.V298.Id.Id Evergreen.V298.Id.UserId) (Evergreen.V298.Thread.LastTypedAt Evergreen.V298.Id.ChannelMessageId)
    , threads : SeqDict.SeqDict (Evergreen.V298.Id.Id Evergreen.V298.Id.ChannelMessageId) Evergreen.V298.Thread.FrontendThread
    , dateDividerDrawings : SeqDict.SeqDict Date.Date (Evergreen.V298.Drawing.Drawing (Evergreen.V298.Id.Id Evergreen.V298.Id.UserId))
    }


type alias FrontendGuild =
    { createdAt : Effect.Time.Posix
    , createdBy : Evergreen.V298.Id.Id Evergreen.V298.Id.UserId
    , name : Evergreen.V298.GuildName.GuildName
    , icon : Maybe Evergreen.V298.FileStatus.FileHash
    , channels : SeqDict.SeqDict (Evergreen.V298.Id.Id Evergreen.V298.Id.ChannelId) FrontendChannel
    , membersAndOwner :
        Evergreen.V298.MembersAndOwner.MembersAndOwner
            (Evergreen.V298.Id.Id Evergreen.V298.Id.UserId)
            { joinedAt : Effect.Time.Posix
            }
    , invites :
        SeqDict.SeqDict
            (Evergreen.V298.SecretId.SecretId Evergreen.V298.Id.InviteLinkId)
            { createdAt : Effect.Time.Posix
            , createdBy : Evergreen.V298.Id.Id Evergreen.V298.Id.UserId
            }
    }


type alias DiscordFrontendChannel =
    { name : Evergreen.V298.ChannelName.ChannelName
    , description : Evergreen.V298.ChannelDescription.ChannelDescription
    , messages : Evergreen.V298.IdArray.IdArray Evergreen.V298.Id.ChannelMessageId (Evergreen.V298.Message.MessageState Evergreen.V298.Id.ChannelMessageId (Evergreen.V298.Discord.Id Evergreen.V298.Discord.UserId))
    , visibleMessages : Evergreen.V298.VisibleMessages.VisibleMessages Evergreen.V298.Id.ChannelMessageId
    , lastTypedAt : SeqDict.SeqDict (Evergreen.V298.Discord.Id Evergreen.V298.Discord.UserId) (Evergreen.V298.Thread.LastTypedAt Evergreen.V298.Id.ChannelMessageId)
    , threads : SeqDict.SeqDict (Evergreen.V298.Id.Id Evergreen.V298.Id.ChannelMessageId) Evergreen.V298.Thread.DiscordFrontendThread
    , dateDividerDrawings : SeqDict.SeqDict Date.Date (Evergreen.V298.Drawing.Drawing (Evergreen.V298.Discord.Id Evergreen.V298.Discord.UserId))
    }


type alias DiscordFrontendGuild =
    { name : Evergreen.V298.GuildName.GuildName
    , icon : Maybe Evergreen.V298.FileStatus.FileHash
    , channels : SeqDict.SeqDict (Evergreen.V298.Discord.Id Evergreen.V298.Discord.ChannelId) DiscordFrontendChannel
    , membersAndOwner :
        Evergreen.V298.MembersAndOwner.MembersAndOwner
            (Evergreen.V298.Discord.Id Evergreen.V298.Discord.UserId)
            { joinedAt : Maybe Effect.Time.Posix
            }
    , stickers : SeqSet.SeqSet (Evergreen.V298.Id.Id Evergreen.V298.Id.StickerId)
    , customEmojis : SeqSet.SeqSet (Evergreen.V298.Id.Id Evergreen.V298.Id.CustomEmojiId)
    }


type JoinGuildError
    = AlreadyJoined
    | InviteIsInvalid


type ServerSecretStatus
    = NotBeingRegenerated (Maybe Effect.Time.Posix)
    | BeingRegenerated
    | RegenerationFailed Effect.Http.Error


type alias AdminData =
    { users : Evergreen.V298.NonemptyDict.NonemptyDict (Evergreen.V298.Id.Id Evergreen.V298.Id.UserId) Evergreen.V298.User.BackendUser
    , emailNotificationsEnabled : Bool
    , twoFactorAuthentication : SeqDict.SeqDict (Evergreen.V298.Id.Id Evergreen.V298.Id.UserId) Effect.Time.Posix
    , privateVapidKey : PrivateVapidKey
    , slackClientSecret : Maybe Evergreen.V298.Slack.ClientSecret
    , openRouterKey : Maybe String
    , cloudflareRealtimeApiToken : Maybe Evergreen.V298.Cloudflare.RealtimeApiToken
    , cloudflareRealtimeAppId : Maybe Evergreen.V298.Cloudflare.AppId
    , cloudflareAccountId : Maybe Evergreen.V298.Cloudflare.AccountId
    , cloudflareAnalyticsApiToken : Maybe Evergreen.V298.Cloudflare.AnalyticsApiToken
    , postmarkKey : Evergreen.V298.Postmark.ApiKey
    , dmChannels : SeqDict.SeqDict Evergreen.V298.DmChannel.DmChannelId AdminData_DmChannel
    , discordDmChannels : SeqDict.SeqDict (Evergreen.V298.Discord.Id Evergreen.V298.Discord.PrivateChannelId) AdminData_DiscordDmChannel
    , discordUsers : SeqDict.SeqDict (Evergreen.V298.Discord.Id Evergreen.V298.Discord.UserId) DiscordUserData_ForAdmin
    , discordGuilds : SeqDict.SeqDict (Evergreen.V298.Discord.Id Evergreen.V298.Discord.GuildId) AdminData_DiscordGuild
    , guilds : SeqDict.SeqDict (Evergreen.V298.Id.Id Evergreen.V298.Id.GuildId) AdminData_Guild
    , deletedGuilds : SeqDict.SeqDict (Evergreen.V298.Id.Id Evergreen.V298.Id.GuildId) AdminData_DeletedGuild
    , loadingDiscordChannels : SeqDict.SeqDict (Evergreen.V298.Discord.Id Evergreen.V298.Discord.UserId) (LoadingDiscordChannel Int)
    , signupsEnabled : Bool
    , discordLinkingEnabled : Bool
    , logs : Evergreen.V298.Pagination.Pagination LogWithTime
    , connections : SeqDict.SeqDict Evergreen.V298.SessionIdHash.SessionIdHash (Evergreen.V298.NonemptyDict.NonemptyDict Effect.Lamdera.ClientId ConnectionData)
    , filesCount : Int
    , toBackendLogs : Array.Array Evergreen.V298.ToBackendLog.ToBackendLogData
    , vulnerabilityChecks : String
    , serverSecretRefreshedAt : ServerSecretStatus
    , websocketCloseEvents : Array.Array WebsocketClosedEvent
    , sessions : SeqDict.SeqDict Evergreen.V298.SessionIdHash.SessionIdHash Evergreen.V298.UserSession.UserSession
    }


type AdminStatus
    = IsAdmin AdminData
    | IsAdminButDataNotLoaded
    | IsNotAdmin


type alias LocalState =
    { adminData : AdminStatus
    , guilds : SeqDict.SeqDict (Evergreen.V298.Id.Id Evergreen.V298.Id.GuildId) FrontendGuild
    , discordGuilds : SeqDict.SeqDict (Evergreen.V298.Discord.Id Evergreen.V298.Discord.GuildId) DiscordFrontendGuild
    , dmChannels : SeqDict.SeqDict (Evergreen.V298.Id.Id Evergreen.V298.Id.UserId) Evergreen.V298.DmChannel.FrontendDmChannel
    , discordDmChannels : SeqDict.SeqDict (Evergreen.V298.Discord.Id Evergreen.V298.Discord.PrivateChannelId) Evergreen.V298.DmChannel.DiscordFrontendDmChannel
    , joinGuildError : Maybe JoinGuildError
    , localUser : Evergreen.V298.User.LocalUser
    , otherSessions : SeqDict.SeqDict Evergreen.V298.SessionIdHash.SessionIdHash Evergreen.V298.UserSession.FrontendUserSession
    , publicVapidKey : String
    , textEditor : Evergreen.V298.TextEditor.LocalState
    , calls : Evergreen.V298.Call.Local
    }


type ChannelStatus
    = ChannelActive
    | ChannelDeleted
        { deletedAt : Effect.Time.Posix
        , deletedBy : Evergreen.V298.Id.Id Evergreen.V298.Id.UserId
        }


type alias BackendChannel =
    { createdAt : Effect.Time.Posix
    , createdBy : Evergreen.V298.Id.Id Evergreen.V298.Id.UserId
    , name : Evergreen.V298.ChannelName.ChannelName
    , description : Evergreen.V298.ChannelDescription.ChannelDescription
    , messages : Evergreen.V298.IdArray.IdArray Evergreen.V298.Id.ChannelMessageId (Evergreen.V298.Message.Message Evergreen.V298.Id.ChannelMessageId (Evergreen.V298.Id.Id Evergreen.V298.Id.UserId))
    , status : ChannelStatus
    , lastTypedAt : SeqDict.SeqDict (Evergreen.V298.Id.Id Evergreen.V298.Id.UserId) (Evergreen.V298.Thread.LastTypedAt Evergreen.V298.Id.ChannelMessageId)
    , threads : SeqDict.SeqDict (Evergreen.V298.Id.Id Evergreen.V298.Id.ChannelMessageId) Evergreen.V298.Thread.BackendThread
    , dateDividerDrawings : SeqDict.SeqDict Date.Date (Evergreen.V298.Drawing.Drawing (Evergreen.V298.Id.Id Evergreen.V298.Id.UserId))
    }


type alias BackendGuild =
    { createdAt : Effect.Time.Posix
    , createdBy : Evergreen.V298.Id.Id Evergreen.V298.Id.UserId
    , name : Evergreen.V298.GuildName.GuildName
    , icon : Maybe Evergreen.V298.FileStatus.FileHash
    , channels : SeqDict.SeqDict (Evergreen.V298.Id.Id Evergreen.V298.Id.ChannelId) BackendChannel
    , membersAndOwner :
        Evergreen.V298.MembersAndOwner.MembersAndOwner
            (Evergreen.V298.Id.Id Evergreen.V298.Id.UserId)
            { joinedAt : Effect.Time.Posix
            }
    , invites :
        SeqDict.SeqDict
            (Evergreen.V298.SecretId.SecretId Evergreen.V298.Id.InviteLinkId)
            { createdAt : Effect.Time.Posix
            , createdBy : Evergreen.V298.Id.Id Evergreen.V298.Id.UserId
            }
    }


type alias DeletedBackendGuild =
    { guild : BackendGuild
    , deletedAt : Effect.Time.Posix
    }


type alias DiscordBackendChannel =
    { name : Evergreen.V298.ChannelName.ChannelName
    , description : Evergreen.V298.ChannelDescription.ChannelDescription
    , messages : Evergreen.V298.IdArray.IdArray Evergreen.V298.Id.ChannelMessageId (Evergreen.V298.Message.Message Evergreen.V298.Id.ChannelMessageId (Evergreen.V298.Discord.Id Evergreen.V298.Discord.UserId))
    , status : ChannelStatus
    , lastTypedAt : SeqDict.SeqDict (Evergreen.V298.Discord.Id Evergreen.V298.Discord.UserId) (Evergreen.V298.Thread.LastTypedAt Evergreen.V298.Id.ChannelMessageId)
    , linkedMessageIds : Evergreen.V298.OneToOne.OneToOne (Evergreen.V298.Discord.Id Evergreen.V298.Discord.MessageId) (Evergreen.V298.Id.Id Evergreen.V298.Id.ChannelMessageId)
    , threads : SeqDict.SeqDict (Evergreen.V298.Id.Id Evergreen.V298.Id.ChannelMessageId) Evergreen.V298.Thread.DiscordBackendThread
    , dateDividerDrawings : SeqDict.SeqDict Date.Date (Evergreen.V298.Drawing.Drawing (Evergreen.V298.Discord.Id Evergreen.V298.Discord.UserId))
    }


type alias DiscordBackendGuild =
    { name : Evergreen.V298.GuildName.GuildName
    , icon : Maybe Evergreen.V298.FileStatus.FileHash
    , channels : SeqDict.SeqDict (Evergreen.V298.Discord.Id Evergreen.V298.Discord.ChannelId) DiscordBackendChannel
    , membersAndOwner :
        Evergreen.V298.MembersAndOwner.MembersAndOwner
            (Evergreen.V298.Discord.Id Evergreen.V298.Discord.UserId)
            { joinedAt : Maybe Effect.Time.Posix
            }
    , stickers : SeqSet.SeqSet (Evergreen.V298.Id.Id Evergreen.V298.Id.StickerId)
    , customEmojis : SeqSet.SeqSet (Evergreen.V298.Id.Id Evergreen.V298.Id.CustomEmojiId)
    }
