module Evergreen.V359.LocalState exposing (..)

import Array
import Date
import Effect.Http
import Effect.Lamdera
import Effect.Time
import Effect.Websocket
import Evergreen.V359.Call
import Evergreen.V359.ChannelDescription
import Evergreen.V359.ChannelName
import Evergreen.V359.Discord
import Evergreen.V359.DiscordUserData
import Evergreen.V359.DmChannel
import Evergreen.V359.DmChannelId
import Evergreen.V359.Drawing
import Evergreen.V359.FileStatus
import Evergreen.V359.Game
import Evergreen.V359.GuildName
import Evergreen.V359.Id
import Evergreen.V359.IdArray
import Evergreen.V359.Log
import Evergreen.V359.MembersAndOwner
import Evergreen.V359.Message
import Evergreen.V359.MessageArray
import Evergreen.V359.NonemptyDict
import Evergreen.V359.OneToOne
import Evergreen.V359.Pagination
import Evergreen.V359.Postmark
import Evergreen.V359.SecretId
import Evergreen.V359.SessionIdHash
import Evergreen.V359.Slack
import Evergreen.V359.TextEditor
import Evergreen.V359.Thread
import Evergreen.V359.ToBackendLog
import Evergreen.V359.User
import Evergreen.V359.UserSession
import Evergreen.V359.VisibleMessages
import SeqDict
import SeqSet


type PrivateVapidKey
    = PrivateVapidKey String


type alias AdminData_DmChannel =
    { messageCount : Int
    , threadCount : Int
    }


type alias AdminData_DiscordDmChannel =
    { members :
        Evergreen.V359.NonemptyDict.NonemptyDict
            (Evergreen.V359.Discord.Id Evergreen.V359.Discord.UserId)
            { messagesSent : Int
            }
    , messageCount : Int
    , firstMessage : Maybe (Evergreen.V359.Message.Message Evergreen.V359.Id.ChannelMessageId (Evergreen.V359.Discord.Id Evergreen.V359.Discord.UserId))
    }


type DiscordUserData_ForAdmin
    = BasicData_ForAdmin
        { user : Evergreen.V359.Discord.PartialUser
        , icon : Maybe Evergreen.V359.FileStatus.FileHash
        }
    | FullData_ForAdmin
        { user : Evergreen.V359.Discord.User
        , linkedTo : Evergreen.V359.Id.Id Evergreen.V359.Id.UserId
        , icon : Maybe Evergreen.V359.FileStatus.FileHash
        , linkedAt : Effect.Time.Posix
        , isLoadingData : Evergreen.V359.DiscordUserData.DiscordUserLoadingData
        }
    | NeedsAuthAgain_ForAdmin
        { user : Evergreen.V359.Discord.User
        , linkedTo : Evergreen.V359.Id.Id Evergreen.V359.Id.UserId
        , icon : Maybe Evergreen.V359.FileStatus.FileHash
        , linkedAt : Effect.Time.Posix
        }


type alias AdminData_DiscordChannel =
    { name : Evergreen.V359.ChannelName.ChannelName
    , messageCount : Int
    , threadCount : Int
    , firstMessage : Maybe (Evergreen.V359.Message.Message Evergreen.V359.Id.ChannelMessageId (Evergreen.V359.Discord.Id Evergreen.V359.Discord.UserId))
    , permissionOverwrites : List Evergreen.V359.Discord.Overwrite
    }


type alias DiscordRole =
    { name : String
    , description : Maybe String
    , permissions : Evergreen.V359.Discord.Permissions
    }


type alias AdminData_DiscordGuild =
    { name : Evergreen.V359.GuildName.GuildName
    , channels : SeqDict.SeqDict (Evergreen.V359.Discord.Id Evergreen.V359.Discord.ChannelId) AdminData_DiscordChannel
    , membersAndOwner :
        Evergreen.V359.MembersAndOwner.MembersAndOwner
            (Evergreen.V359.Discord.Id Evergreen.V359.Discord.UserId)
            { joinedAt : Maybe Effect.Time.Posix
            , roles : SeqSet.SeqSet (Evergreen.V359.Discord.Id Evergreen.V359.Discord.RoleId)
            }
    , roles : SeqDict.SeqDict (Evergreen.V359.Discord.Id Evergreen.V359.Discord.RoleId) DiscordRole
    }


type alias AdminData_GuildChannel =
    { name : Evergreen.V359.ChannelName.ChannelName
    , messageCount : Int
    }


type alias AdminData_Guild =
    { name : Evergreen.V359.GuildName.GuildName
    , channels : SeqDict.SeqDict (Evergreen.V359.Id.Id Evergreen.V359.Id.ChannelId) AdminData_GuildChannel
    , memberCount : Int
    , owner : Evergreen.V359.Id.Id Evergreen.V359.Id.UserId
    }


type alias AdminData_DeletedGuild =
    { name : Evergreen.V359.GuildName.GuildName
    , owner : Evergreen.V359.Id.Id Evergreen.V359.Id.UserId
    , memberCount : Int
    , deletedAt : Effect.Time.Posix
    }


type LoadingDiscordChannelStep messages
    = LoadingDiscordChannelMessages
    | LoadingDiscordChannelMessagesFailed Evergreen.V359.Discord.HttpError
    | LoadingDiscordChannelAttachments Effect.Time.Posix messages


type LoadingDiscordChannel messages
    = LoadingDiscordDmChannel Effect.Time.Posix (Evergreen.V359.Discord.Id Evergreen.V359.Discord.PrivateChannelId) (LoadingDiscordChannelStep messages)
    | LoadingDiscordGuildChannel Effect.Time.Posix (Evergreen.V359.Discord.Id Evergreen.V359.Discord.GuildId) (Evergreen.V359.Discord.Id Evergreen.V359.Discord.ChannelId) (LoadingDiscordChannelStep messages)


type alias LogWithTime =
    { time : Effect.Time.Posix
    , log : Evergreen.V359.Log.Log
    , isHidden : Bool
    }


type LastRequest
    = NoRequestsMade
    | LastRequest Effect.Time.Posix


type CallStatus
    = NotInCall
    | ConnectedToCall Evergreen.V359.Call.CallId


type alias ConnectionData =
    { lastRequest : LastRequest
    , call : CallStatus
    , remoteCallData : Evergreen.V359.Call.RemoteCallData
    , currentlyViewing : Evergreen.V359.UserSession.Viewing
    }


type WebsocketClosedEvent
    = WebsocketClosed_CloseAndReopenForUser (Evergreen.V359.Discord.Id Evergreen.V359.Discord.UserId) Effect.Time.Posix
    | WebsocketClosed_UnlinkDiscordUser (Evergreen.V359.Discord.Id Evergreen.V359.Discord.UserId) Effect.Time.Posix
    | WebsocketClosed_ClosedByBackendForUser (Evergreen.V359.Discord.Id Evergreen.V359.Discord.UserId) Effect.Time.Posix
    | WebsocketClosed_ListenCloseEvent (Evergreen.V359.Discord.Id Evergreen.V359.Discord.UserId) Effect.Websocket.CloseEventCode String Effect.Time.Posix


type WordSpellingGameStatus
    = WordSpellingGameStatus_NotLoaded
    | WordSpellingGameStatus_Loading
    | WordSpellingGameStatus_Error Effect.Http.Error
    | WordSpellingGameStatus_Loaded


type alias Archived =
    { archivedAt : Effect.Time.Posix
    , archivedBy : Evergreen.V359.Id.Id Evergreen.V359.Id.UserId
    }


type alias FrontendChannel =
    { createdAt : Effect.Time.Posix
    , createdBy : Evergreen.V359.Id.Id Evergreen.V359.Id.UserId
    , name : Evergreen.V359.ChannelName.ChannelName
    , description : Evergreen.V359.ChannelDescription.ChannelDescription
    , messages : Evergreen.V359.MessageArray.MessageArray Evergreen.V359.Id.ChannelMessageId (Evergreen.V359.Message.Message Evergreen.V359.Id.ChannelMessageId (Evergreen.V359.Id.Id Evergreen.V359.Id.UserId))
    , visibleMessages : Evergreen.V359.VisibleMessages.VisibleMessages Evergreen.V359.Id.ChannelMessageId
    , isArchived : Maybe Archived
    , lastTypedAt : SeqDict.SeqDict (Evergreen.V359.Id.Id Evergreen.V359.Id.UserId) (Evergreen.V359.Thread.LastTypedAt Evergreen.V359.Id.ChannelMessageId)
    , threads : SeqDict.SeqDict (Evergreen.V359.Id.Id Evergreen.V359.Id.ChannelMessageId) Evergreen.V359.Thread.FrontendThread
    , dateDividerDrawings : SeqDict.SeqDict Date.Date (Evergreen.V359.Drawing.Drawing (Evergreen.V359.Id.Id Evergreen.V359.Id.UserId))
    , games : SeqDict.SeqDict (Evergreen.V359.Id.Id Evergreen.V359.Id.ChannelMessageId) Evergreen.V359.Game.MatchData
    }


type alias FrontendGuild =
    { createdAt : Effect.Time.Posix
    , createdBy : Evergreen.V359.Id.Id Evergreen.V359.Id.UserId
    , name : Evergreen.V359.GuildName.GuildName
    , icon : Maybe Evergreen.V359.FileStatus.FileHash
    , channels : SeqDict.SeqDict (Evergreen.V359.Id.Id Evergreen.V359.Id.ChannelId) FrontendChannel
    , membersAndOwner :
        Evergreen.V359.MembersAndOwner.MembersAndOwner
            (Evergreen.V359.Id.Id Evergreen.V359.Id.UserId)
            { joinedAt : Effect.Time.Posix
            }
    , invites :
        SeqDict.SeqDict
            (Evergreen.V359.SecretId.SecretId Evergreen.V359.Id.InviteLinkId)
            { createdAt : Effect.Time.Posix
            , createdBy : Evergreen.V359.Id.Id Evergreen.V359.Id.UserId
            }
    }


type alias DiscordFrontendChannel =
    { name : Evergreen.V359.ChannelName.ChannelName
    , description : Evergreen.V359.ChannelDescription.ChannelDescription
    , isForum : Bool
    , messages : Evergreen.V359.MessageArray.MessageArray Evergreen.V359.Id.ChannelMessageId (Evergreen.V359.Message.Message Evergreen.V359.Id.ChannelMessageId (Evergreen.V359.Discord.Id Evergreen.V359.Discord.UserId))
    , visibleMessages : Evergreen.V359.VisibleMessages.VisibleMessages Evergreen.V359.Id.ChannelMessageId
    , lastTypedAt : SeqDict.SeqDict (Evergreen.V359.Discord.Id Evergreen.V359.Discord.UserId) (Evergreen.V359.Thread.LastTypedAt Evergreen.V359.Id.ChannelMessageId)
    , threads : SeqDict.SeqDict (Evergreen.V359.Id.Id Evergreen.V359.Id.ChannelMessageId) Evergreen.V359.Thread.DiscordFrontendThread
    , dateDividerDrawings : SeqDict.SeqDict Date.Date (Evergreen.V359.Drawing.Drawing (Evergreen.V359.Discord.Id Evergreen.V359.Discord.UserId))
    , permissionOverwrites : List Evergreen.V359.Discord.Overwrite
    }


type alias DiscordFrontendGuild =
    { name : Evergreen.V359.GuildName.GuildName
    , icon : Maybe Evergreen.V359.FileStatus.FileHash
    , channels : SeqDict.SeqDict (Evergreen.V359.Discord.Id Evergreen.V359.Discord.ChannelId) DiscordFrontendChannel
    , membersAndOwner :
        Evergreen.V359.MembersAndOwner.MembersAndOwner
            (Evergreen.V359.Discord.Id Evergreen.V359.Discord.UserId)
            { joinedAt : Maybe Effect.Time.Posix
            , roles : SeqSet.SeqSet (Evergreen.V359.Discord.Id Evergreen.V359.Discord.RoleId)
            }
    , stickers : SeqSet.SeqSet (Evergreen.V359.Id.Id Evergreen.V359.Id.StickerId)
    , customEmojis : SeqSet.SeqSet (Evergreen.V359.Id.Id Evergreen.V359.Id.CustomEmojiId)
    , roles : SeqDict.SeqDict (Evergreen.V359.Discord.Id Evergreen.V359.Discord.RoleId) DiscordRole
    }


type JoinGuildError
    = AlreadyJoined
    | InviteIsInvalid


type ServerSecretStatus
    = NotBeingRegenerated (Maybe Effect.Time.Posix)
    | BeingRegenerated
    | RegenerationFailed Effect.Http.Error


type alias AdminData =
    { users : Evergreen.V359.NonemptyDict.NonemptyDict (Evergreen.V359.Id.Id Evergreen.V359.Id.UserId) Evergreen.V359.User.BackendUser
    , emailNotificationsEnabled : Bool
    , twoFactorAuthentication : SeqDict.SeqDict (Evergreen.V359.Id.Id Evergreen.V359.Id.UserId) Effect.Time.Posix
    , privateVapidKey : PrivateVapidKey
    , slackClientSecret : Maybe Evergreen.V359.Slack.ClientSecret
    , openRouterKey : Maybe String
    , postmarkKey : Evergreen.V359.Postmark.ApiKey
    , dmChannels : SeqDict.SeqDict Evergreen.V359.DmChannelId.DmChannelId AdminData_DmChannel
    , discordDmChannels : SeqDict.SeqDict (Evergreen.V359.Discord.Id Evergreen.V359.Discord.PrivateChannelId) AdminData_DiscordDmChannel
    , discordUsers : SeqDict.SeqDict (Evergreen.V359.Discord.Id Evergreen.V359.Discord.UserId) DiscordUserData_ForAdmin
    , discordGuilds : SeqDict.SeqDict (Evergreen.V359.Discord.Id Evergreen.V359.Discord.GuildId) AdminData_DiscordGuild
    , guilds : SeqDict.SeqDict (Evergreen.V359.Id.Id Evergreen.V359.Id.GuildId) AdminData_Guild
    , deletedGuilds : SeqDict.SeqDict (Evergreen.V359.Id.Id Evergreen.V359.Id.GuildId) AdminData_DeletedGuild
    , loadingDiscordChannels : SeqDict.SeqDict (Evergreen.V359.Discord.Id Evergreen.V359.Discord.UserId) (LoadingDiscordChannel Int)
    , signupsEnabled : Bool
    , discordLinkingEnabled : Bool
    , logs : Evergreen.V359.Pagination.Pagination LogWithTime
    , connections : SeqDict.SeqDict Evergreen.V359.SessionIdHash.SessionIdHash (Evergreen.V359.NonemptyDict.NonemptyDict Effect.Lamdera.ClientId ConnectionData)
    , filesCount : Int
    , toBackendLogs : Array.Array Evergreen.V359.ToBackendLog.ToBackendLogData
    , vulnerabilityChecks : String
    , serverSecretRefreshedAt : ServerSecretStatus
    , websocketCloseEvents : Array.Array WebsocketClosedEvent
    , sessions : SeqDict.SeqDict Evergreen.V359.SessionIdHash.SessionIdHash Evergreen.V359.UserSession.UserSession
    , wordSpellingGameEnglish : WordSpellingGameStatus
    , wordSpellingGameSwedish : WordSpellingGameStatus
    }


type AdminStatus
    = IsAdmin AdminData
    | IsAdminButDataNotLoaded
    | IsNotAdmin


type alias LocalState =
    { adminData : AdminStatus
    , guilds : SeqDict.SeqDict (Evergreen.V359.Id.Id Evergreen.V359.Id.GuildId) FrontendGuild
    , discordGuilds : SeqDict.SeqDict (Evergreen.V359.Discord.Id Evergreen.V359.Discord.GuildId) DiscordFrontendGuild
    , dmChannels : SeqDict.SeqDict (Evergreen.V359.Id.Id Evergreen.V359.Id.UserId) Evergreen.V359.DmChannel.FrontendDmChannel
    , discordDmChannels : SeqDict.SeqDict (Evergreen.V359.Discord.Id Evergreen.V359.Discord.PrivateChannelId) Evergreen.V359.DmChannel.DiscordFrontendDmChannel
    , joinGuildError : Maybe JoinGuildError
    , localUser : Evergreen.V359.User.LocalUser
    , otherSessions : SeqDict.SeqDict Evergreen.V359.SessionIdHash.SessionIdHash Evergreen.V359.UserSession.FrontendUserSession
    , publicVapidKey : String
    , textEditor : Evergreen.V359.TextEditor.LocalState
    , calls : Evergreen.V359.Call.Local
    }


type ChannelStatus
    = ChannelActive
    | ChannelDeleted
        { deletedAt : Effect.Time.Posix
        , deletedBy : Evergreen.V359.Id.Id Evergreen.V359.Id.UserId
        }


type alias BackendChannel =
    { createdAt : Effect.Time.Posix
    , createdBy : Evergreen.V359.Id.Id Evergreen.V359.Id.UserId
    , name : Evergreen.V359.ChannelName.ChannelName
    , description : Evergreen.V359.ChannelDescription.ChannelDescription
    , messages : Evergreen.V359.IdArray.IdArray Evergreen.V359.Id.ChannelMessageId (Evergreen.V359.Message.Message Evergreen.V359.Id.ChannelMessageId (Evergreen.V359.Id.Id Evergreen.V359.Id.UserId))
    , status : ChannelStatus
    , lastTypedAt : SeqDict.SeqDict (Evergreen.V359.Id.Id Evergreen.V359.Id.UserId) (Evergreen.V359.Thread.LastTypedAt Evergreen.V359.Id.ChannelMessageId)
    , threads : SeqDict.SeqDict (Evergreen.V359.Id.Id Evergreen.V359.Id.ChannelMessageId) Evergreen.V359.Thread.BackendThread
    , dateDividerDrawings : SeqDict.SeqDict Date.Date (Evergreen.V359.Drawing.Drawing (Evergreen.V359.Id.Id Evergreen.V359.Id.UserId))
    , games : SeqDict.SeqDict (Evergreen.V359.Id.Id Evergreen.V359.Id.ChannelMessageId) Evergreen.V359.Game.BackendGameData
    }


type alias BackendGuild =
    { createdAt : Effect.Time.Posix
    , createdBy : Evergreen.V359.Id.Id Evergreen.V359.Id.UserId
    , name : Evergreen.V359.GuildName.GuildName
    , icon : Maybe Evergreen.V359.FileStatus.FileHash
    , channels : SeqDict.SeqDict (Evergreen.V359.Id.Id Evergreen.V359.Id.ChannelId) BackendChannel
    , membersAndOwner :
        Evergreen.V359.MembersAndOwner.MembersAndOwner
            (Evergreen.V359.Id.Id Evergreen.V359.Id.UserId)
            { joinedAt : Effect.Time.Posix
            }
    , invites :
        SeqDict.SeqDict
            (Evergreen.V359.SecretId.SecretId Evergreen.V359.Id.InviteLinkId)
            { createdAt : Effect.Time.Posix
            , createdBy : Evergreen.V359.Id.Id Evergreen.V359.Id.UserId
            }
    }


type alias DeletedBackendGuild =
    { guild : BackendGuild
    , deletedAt : Effect.Time.Posix
    }


type alias DiscordBackendChannel =
    { name : Evergreen.V359.ChannelName.ChannelName
    , description : Evergreen.V359.ChannelDescription.ChannelDescription
    , isForum : Bool
    , messages : Evergreen.V359.IdArray.IdArray Evergreen.V359.Id.ChannelMessageId (Evergreen.V359.Message.Message Evergreen.V359.Id.ChannelMessageId (Evergreen.V359.Discord.Id Evergreen.V359.Discord.UserId))
    , status : ChannelStatus
    , lastTypedAt : SeqDict.SeqDict (Evergreen.V359.Discord.Id Evergreen.V359.Discord.UserId) (Evergreen.V359.Thread.LastTypedAt Evergreen.V359.Id.ChannelMessageId)
    , linkedMessageIds : Evergreen.V359.OneToOne.OneToOne (Evergreen.V359.Discord.Id Evergreen.V359.Discord.MessageId) (Evergreen.V359.Id.Id Evergreen.V359.Id.ChannelMessageId)
    , threads : SeqDict.SeqDict (Evergreen.V359.Id.Id Evergreen.V359.Id.ChannelMessageId) Evergreen.V359.Thread.DiscordBackendThread
    , dateDividerDrawings : SeqDict.SeqDict Date.Date (Evergreen.V359.Drawing.Drawing (Evergreen.V359.Discord.Id Evergreen.V359.Discord.UserId))
    , permissionOverwrites : List Evergreen.V359.Discord.Overwrite
    }


type alias DiscordBackendGuild =
    { name : Evergreen.V359.GuildName.GuildName
    , icon : Maybe Evergreen.V359.FileStatus.FileHash
    , channels : SeqDict.SeqDict (Evergreen.V359.Discord.Id Evergreen.V359.Discord.ChannelId) DiscordBackendChannel
    , membersAndOwner :
        Evergreen.V359.MembersAndOwner.MembersAndOwner
            (Evergreen.V359.Discord.Id Evergreen.V359.Discord.UserId)
            { joinedAt : Maybe Effect.Time.Posix
            , roles : SeqSet.SeqSet (Evergreen.V359.Discord.Id Evergreen.V359.Discord.RoleId)
            }
    , stickers : SeqSet.SeqSet (Evergreen.V359.Id.Id Evergreen.V359.Id.StickerId)
    , customEmojis : SeqSet.SeqSet (Evergreen.V359.Id.Id Evergreen.V359.Id.CustomEmojiId)
    , roles : SeqDict.SeqDict (Evergreen.V359.Discord.Id Evergreen.V359.Discord.RoleId) DiscordRole
    }


type alias DiscordThreadReload =
    { threadId : Evergreen.V359.Discord.Id Evergreen.V359.Discord.ChannelId
    , messages : List Evergreen.V359.Discord.Message
    }


type alias DiscordChannelReload =
    { messages : List Evergreen.V359.Discord.Message
    , threads : List DiscordThreadReload
    }
