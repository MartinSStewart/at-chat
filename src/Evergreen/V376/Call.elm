module Evergreen.V376.Call exposing (..)

import Bytes
import Effect.Lamdera
import Effect.Time
import Evergreen.V376.Id
import Evergreen.V376.IdString
...
type alias MediaDevice =
    { deviceId : Evergreen.V376.IdString.IdString MediaDeviceId
    , groupId : String
    , kind : DeviceKind
    , label : String
    }
...
type CallId
    = DmRoomId Evergreen.V376.Id.Viewing_DmId
    | GuildRoomId Evergreen.V376.Id.Viewing_ChannelId


type alias ConnectionId =
    { roomId : CallId
    , otherClientId : ( Evergreen.V376.Id.Id Evergreen.V376.Id.UserId, Effect.Lamdera.ClientId )
    }
...
type FromJs
    = FromJs_GotUserMediaDevices (List MediaDevice) (List (Evergreen.V376.IdString.IdString MediaDeviceId))
...
type Msg
    = SelectedAudioInputDevice (Evergreen.V376.IdString.IdString MediaDeviceId)
    | SelectedVideoInputDevice (Evergreen.V376.IdString.IdString MediaDeviceId)
    | PressedToggleMute
...
type alias Local =
    ...
    , voiceChats : SeqDict.SeqDict CallId (Evergreen.V376.NonemptyDict.NonemptyDict ( Evergreen.V376.Id.Id Evergreen.V376.Id.UserId, Effect.Lamdera.ClientId ) RemoteCallData)
    }
...
type alias Model =
    ...
    , selectedAudioInputDevice : Maybe (Evergreen.V376.IdString.IdString MediaDeviceId)
    , selectedVideoInputDevice : Maybe (Evergreen.V376.IdString.IdString MediaDeviceId)
    ...
    , volume : SeqDict.SeqDict ( Evergreen.V376.Id.Id Evergreen.V376.Id.UserId, Effect.Lamdera.ClientId ) Float
