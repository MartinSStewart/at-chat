module Evergreen.V373.LocalState exposing (..)

import Array
import Date
import Effect.Http
import Effect.Lamdera
import Effect.Time
import Effect.Websocket
import Evergreen.V373.Call
import Evergreen.V373.ChannelDescription
import Evergreen.V373.ChannelName
import Evergreen.V373.Discord
import Evergreen.V373.DiscordUserData
import Evergreen.V373.DmChannel
import Evergreen.V373.DmChannelId
import Evergreen.V373.Drawing
import Evergreen.V373.FileStatus
import Evergreen.V373.Game
import Evergreen.V373.GuildName
import Evergreen.V373.Id
import Evergreen.V373.IdArray
import Evergreen.V373.Log
import Evergreen.V373.MembersAndOwner
import Evergreen.V373.Message
import Evergreen.V373.MessageArray
import Evergreen.V373.NonemptyDict
import Evergreen.V373.OneToOne
import Evergreen.V373.Pagination
import Evergreen.V373.Postmark
import Evergreen.V373.SecretId
import Evergreen.V373.SessionIdHash
import Evergreen.V373.Slack
import Evergreen.V373.TextEditor
import Evergreen.V373.Thread
import Evergreen.V373.ToBackendLog
import Evergreen.V373.User
import Evergreen.V373.UserSession
import Evergreen.V373.VisibleMessages
import SeqDict
import SeqSet


type PrivateVapidKey
    = PrivateVapidKey String


type alias AdminData_DmChannel =
    { messageCount : Int
    , threadCount : Int
    }


type alias AdminData_DiscordDmChannel =
    { members :
        Evergreen.V373.NonemptyDict.NonemptyDict
            (Evergreen.V373.Discord.Id Evergreen.V373.Discord.UserId)
            { messagesSent : Int
            }
    , messageCount : Int
    , firstMessage : Maybe (Evergreen.V373.Message.Message Evergreen.V373.Id.ChannelMessageId (Evergreen.V373.Discord.Id Evergreen.V373.Discord.UserId))
    }


type alias DiscordGatewayStatus =
    { websocketIsOpen : Bool
    , failedReconnectAttempts : Int
    }


type DiscordUserData_ForAdmin
    = BasicData_ForAdmin
        { user : Evergreen.V373.Discord.PartialUser
        , icon : Maybe Evergreen.V373.FileStatus.FileHash
        }
    | FullData_ForAdmin
        { user : Evergreen.V373.Discord.User
        , linkedTo : Evergreen.V373.Id.Id Evergreen.V373.Id.UserId
        , icon : Maybe Evergreen.V373.FileStatus.FileHash
        , linkedAt : Effect.Time.Posix
        , isLoadingData : Evergreen.V373.DiscordUserData.DiscordUserLoadingData
        , gateway : DiscordGatewayStatus
        }
    | NeedsAuthAgain_ForAdmin
        { user : Evergreen.V373.Discord.User
        , linkedTo : Evergreen.V373.Id.Id Evergreen.V373.Id.UserId
        , icon : Maybe Evergreen.V373.FileStatus.FileHash
        , linkedAt : Effect.Time.Posix
        }


type alias AdminData_DiscordChannel =
    { name : Evergreen.V373.ChannelName.ChannelName
    , messageCount : Int
    , threadCount : Int
    , firstMessage : Maybe (Evergreen.V373.Message.Message Evergreen.V373.Id.ChannelMessageId (Evergreen.V373.Discord.Id Evergreen.V373.Discord.UserId))
    , permissionOverwrites : List Evergreen.V373.Discord.Overwrite
    }


type alias DiscordRole =
    { name : String
    , description : Maybe String
    , permissions : Evergreen.V373.Discord.Permissions
    }


type alias AdminData_DiscordGuild =
    { name : Evergreen.V373.GuildName.GuildName
    , channels : SeqDict.SeqDict (Evergreen.V373.Discord.Id Evergreen.V373.Discord.ChannelId) AdminData_DiscordChannel
    , membersAndOwner :
        Evergreen.V373.MembersAndOwner.MembersAndOwner
            (Evergreen.V373.Discord.Id Evergreen.V373.Discord.UserId)
            { joinedAt : Maybe Effect.Time.Posix
            , roles : SeqSet.SeqSet (Evergreen.V373.Discord.Id Evergreen.V373.Discord.RoleId)
            }
    , roles : SeqDict.SeqDict (Evergreen.V373.Discord.Id Evergreen.V373.Discord.RoleId) DiscordRole
    }


type alias AdminData_GuildChannel =
    { name : Evergreen.V373.ChannelName.ChannelName
    , messageCount : Int
    }


type alias AdminData_Guild =
    { name : Evergreen.V373.GuildName.GuildName
    , channels : SeqDict.SeqDict (Evergreen.V373.Id.Id Evergreen.V373.Id.ChannelId) AdminData_GuildChannel
    , memberCount : Int
    , owner : Evergreen.V373.Id.Id Evergreen.V373.Id.UserId
    }


type alias AdminData_DeletedGuild =
    { name : Evergreen.V373.GuildName.GuildName
    , owner : Evergreen.V373.Id.Id Evergreen.V373.Id.UserId
    , memberCount : Int
    , deletedAt : Effect.Time.Posix
    }


type LoadingDiscordChannelStep messages
    = LoadingDiscordChannelMessages
    | LoadingDiscordChannelMessagesFailed Evergreen.V373.Discord.HttpError
    | LoadingDiscordChannelAttachments Effect.Time.Posix messages


type LoadingDiscordChannel messages
    = LoadingDiscordDmChannel Effect.Time.Posix (Evergreen.V373.Discord.Id Evergreen.V373.Discord.PrivateChannelId) (LoadingDiscordChannelStep messages)
    | LoadingDiscordGuildChannel Effect.Time.Posix (Evergreen.V373.Discord.Id Evergreen.V373.Discord.GuildId) (Evergreen.V373.Discord.Id Evergreen.V373.Discord.ChannelId) (LoadingDiscordChannelStep messages)


type alias LogWithTime =
    { time : Effect.Time.Posix
    , log : Evergreen.V373.Log.Log
    , isHidden : Bool
    }


type LastRequest
    = NoRequestsMade
    | LastRequest Effect.Time.Posix


type CallStatus
    = NotInCall
    | ConnectedToCall Evergreen.V373.Call.CallId


type alias ConnectionData =
    { lastRequest : LastRequest
    , call : CallStatus
    , remoteCallData : Evergreen.V373.Call.RemoteCallData
    , currentlyViewing : Evergreen.V373.UserSession.Viewing
    }


type BackupContents
    = FullBackup
    | SubsetBackup


type alias LastBackup =
    { createdAt : Effect.Time.Posix
    , contents : BackupContents
    }


type WebsocketClosedEvent
    = WebsocketClosed_CloseAndReopenForUser (Evergreen.V373.Discord.Id Evergreen.V373.Discord.UserId) Effect.Time.Posix
    | WebsocketClosed_UnlinkDiscordUser (Evergreen.V373.Discord.Id Evergreen.V373.Discord.UserId) Effect.Time.Posix
    | WebsocketClosed_ClosedByBackendForUser (Evergreen.V373.Discord.Id Evergreen.V373.Discord.UserId) Effect.Time.Posix
    | WebsocketClosed_ListenCloseEvent (Evergreen.V373.Discord.Id Evergreen.V373.Discord.UserId) Effect.Websocket.CloseEventCode String Effect.Time.Posix


type WordSpellingGameStatus
    = WordSpellingGameStatus_NotLoaded
    | WordSpellingGameStatus_Loading
    | WordSpellingGameStatus_Error Effect.Http.Error
    | WordSpellingGameStatus_Loaded


type alias Archived =
    { archivedAt : Effect.Time.Posix
    , archivedBy : Evergreen.V373.Id.Id Evergreen.V373.Id.UserId
    }


type alias FrontendChannel =
    { createdAt : Effect.Time.Posix
    , createdBy : Evergreen.V373.Id.Id Evergreen.V373.Id.UserId
    , name : Evergreen.V373.ChannelName.ChannelName
    , description : Evergreen.V373.ChannelDescription.ChannelDescription
    , messages : Evergreen.V373.MessageArray.MessageArray Evergreen.V373.Id.ChannelMessageId (Evergreen.V373.Id.Id Evergreen.V373.Id.UserId)
    , visibleMessages : Evergreen.V373.VisibleMessages.VisibleMessages Evergreen.V373.Id.ChannelMessageId
    , isArchived : Maybe Archived
    , lastTypedAt : SeqDict.SeqDict (Evergreen.V373.Id.Id Evergreen.V373.Id.UserId) (Evergreen.V373.Thread.LastTypedAt Evergreen.V373.Id.ChannelMessageId)
    , threads : SeqDict.SeqDict (Evergreen.V373.Id.Id Evergreen.V373.Id.ChannelMessageId) Evergreen.V373.Thread.FrontendThread
    , dateDividerDrawings : SeqDict.SeqDict Date.Date (Evergreen.V373.Drawing.Drawing (Evergreen.V373.Id.Id Evergreen.V373.Id.UserId))
    , games : SeqDict.SeqDict (Evergreen.V373.Id.Id Evergreen.V373.Id.ChannelMessageId) Evergreen.V373.Game.MatchData
    }


type alias FrontendGuild =
    { createdAt : Effect.Time.Posix
    , createdBy : Evergreen.V373.Id.Id Evergreen.V373.Id.UserId
    , name : Evergreen.V373.GuildName.GuildName
    , icon : Maybe Evergreen.V373.FileStatus.FileHash
    , channels : SeqDict.SeqDict (Evergreen.V373.Id.Id Evergreen.V373.Id.ChannelId) FrontendChannel
    , membersAndOwner :
        Evergreen.V373.MembersAndOwner.MembersAndOwner
            (Evergreen.V373.Id.Id Evergreen.V373.Id.UserId)
            { joinedAt : Effect.Time.Posix
            }
    , invites :
        SeqDict.SeqDict
            (Evergreen.V373.SecretId.SecretId Evergreen.V373.Id.InviteLinkId)
            { createdAt : Effect.Time.Posix
            , createdBy : Evergreen.V373.Id.Id Evergreen.V373.Id.UserId
            }
    }


type alias DiscordFrontendChannel =
    { name : Evergreen.V373.ChannelName.ChannelName
    , description : Evergreen.V373.ChannelDescription.ChannelDescription
    , isForum : Bool
    , messages : Evergreen.V373.MessageArray.MessageArray Evergreen.V373.Id.ChannelMessageId (Evergreen.V373.Discord.Id Evergreen.V373.Discord.UserId)
    , visibleMessages : Evergreen.V373.VisibleMessages.VisibleMessages Evergreen.V373.Id.ChannelMessageId
    , lastTypedAt : SeqDict.SeqDict (Evergreen.V373.Discord.Id Evergreen.V373.Discord.UserId) (Evergreen.V373.Thread.LastTypedAt Evergreen.V373.Id.ChannelMessageId)
    , threads : SeqDict.SeqDict (Evergreen.V373.Id.Id Evergreen.V373.Id.ChannelMessageId) Evergreen.V373.Thread.DiscordFrontendThread
    , dateDividerDrawings : SeqDict.SeqDict Date.Date (Evergreen.V373.Drawing.Drawing (Evergreen.V373.Discord.Id Evergreen.V373.Discord.UserId))
    , permissionOverwrites : List Evergreen.V373.Discord.Overwrite
    }


type alias DiscordFrontendGuild =
    { name : Evergreen.V373.GuildName.GuildName
    , icon : Maybe Evergreen.V373.FileStatus.FileHash
    , channels : SeqDict.SeqDict (Evergreen.V373.Discord.Id Evergreen.V373.Discord.ChannelId) DiscordFrontendChannel
    , membersAndOwner :
        Evergreen.V373.MembersAndOwner.MembersAndOwner
            (Evergreen.V373.Discord.Id Evergreen.V373.Discord.UserId)
            { joinedAt : Maybe Effect.Time.Posix
            , roles : SeqSet.SeqSet (Evergreen.V373.Discord.Id Evergreen.V373.Discord.RoleId)
            }
    , stickers : SeqSet.SeqSet (Evergreen.V373.Id.Id Evergreen.V373.Id.StickerId)
    , customEmojis : SeqSet.SeqSet (Evergreen.V373.Id.Id Evergreen.V373.Id.CustomEmojiId)
    , roles : SeqDict.SeqDict (Evergreen.V373.Discord.Id Evergreen.V373.Discord.RoleId) DiscordRole
    }


type JoinGuildError
    = AlreadyJoined
    | InviteIsInvalid


type ServerSecretStatus
    = NotBeingRegenerated (Maybe Effect.Time.Posix)
    | BeingRegenerated
    | RegenerationFailed Effect.Http.Error


type alias AdminData =
    { users : Evergreen.V373.NonemptyDict.NonemptyDict (Evergreen.V373.Id.Id Evergreen.V373.Id.UserId) Evergreen.V373.User.BackendUser
    , emailNotificationsEnabled : Bool
    , twoFactorAuthentication : SeqDict.SeqDict (Evergreen.V373.Id.Id Evergreen.V373.Id.UserId) Effect.Time.Posix
    , privateVapidKey : PrivateVapidKey
    , slackClientSecret : Maybe Evergreen.V373.Slack.ClientSecret
    , openRouterKey : Maybe String
    , postmarkKey : Evergreen.V373.Postmark.ApiKey
    , dmChannels : SeqDict.SeqDict Evergreen.V373.DmChannelId.DmChannelId AdminData_DmChannel
    , discordDmChannels : SeqDict.SeqDict (Evergreen.V373.Discord.Id Evergreen.V373.Discord.PrivateChannelId) AdminData_DiscordDmChannel
    , discordUsers : SeqDict.SeqDict (Evergreen.V373.Discord.Id Evergreen.V373.Discord.UserId) DiscordUserData_ForAdmin
    , discordGuilds : SeqDict.SeqDict (Evergreen.V373.Discord.Id Evergreen.V373.Discord.GuildId) AdminData_DiscordGuild
    , guilds : SeqDict.SeqDict (Evergreen.V373.Id.Id Evergreen.V373.Id.GuildId) AdminData_Guild
    , deletedGuilds : SeqDict.SeqDict (Evergreen.V373.Id.Id Evergreen.V373.Id.GuildId) AdminData_DeletedGuild
    , loadingDiscordChannels : SeqDict.SeqDict (Evergreen.V373.Discord.Id Evergreen.V373.Discord.UserId) (LoadingDiscordChannel Int)
    , signupsEnabled : Bool
    , discordLinkingEnabled : Bool
    , logs : Evergreen.V373.Pagination.Pagination LogWithTime
    , connections : SeqDict.SeqDict Evergreen.V373.SessionIdHash.SessionIdHash (Evergreen.V373.NonemptyDict.NonemptyDict Effect.Lamdera.ClientId ConnectionData)
    , filesCount : Int
    , toBackendLogs : Array.Array Evergreen.V373.ToBackendLog.ToBackendLogData
    , vulnerabilityChecks : String
    , serverSecretRefreshedAt : ServerSecretStatus
    , lastBackup : Maybe LastBackup
    , websocketCloseEvents : Array.Array WebsocketClosedEvent
    , sessions : SeqDict.SeqDict Evergreen.V373.SessionIdHash.SessionIdHash Evergreen.V373.UserSession.UserSession
    , wordSpellingGameEnglish : WordSpellingGameStatus
    , wordSpellingGameSwedish : WordSpellingGameStatus
    }


type AdminStatus
    = IsAdmin AdminData
    | IsAdminButDataNotLoaded
    | IsNotAdmin


type alias LocalState =
    { adminData : AdminStatus
    , guilds : SeqDict.SeqDict (Evergreen.V373.Id.Id Evergreen.V373.Id.GuildId) FrontendGuild
    , discordGuilds : SeqDict.SeqDict (Evergreen.V373.Discord.Id Evergreen.V373.Discord.GuildId) DiscordFrontendGuild
    , dmChannels : SeqDict.SeqDict (Evergreen.V373.Id.Id Evergreen.V373.Id.UserId) Evergreen.V373.DmChannel.FrontendDmChannel
    , discordDmChannels : SeqDict.SeqDict (Evergreen.V373.Discord.Id Evergreen.V373.Discord.PrivateChannelId) Evergreen.V373.DmChannel.DiscordFrontendDmChannel
    , joinGuildError : Maybe JoinGuildError
    , localUser : Evergreen.V373.User.LocalUser
    , otherSessions : SeqDict.SeqDict Evergreen.V373.SessionIdHash.SessionIdHash Evergreen.V373.UserSession.FrontendUserSession
    , publicVapidKey : String
    , textEditor : Evergreen.V373.TextEditor.LocalState
    , calls : Evergreen.V373.Call.Local
    }


type ChannelStatus
    = ChannelActive
    | ChannelDeleted
        { deletedAt : Effect.Time.Posix
        , deletedBy : Evergreen.V373.Id.Id Evergreen.V373.Id.UserId
        }


type alias BackendChannel =
    { createdAt : Effect.Time.Posix
    , createdBy : Evergreen.V373.Id.Id Evergreen.V373.Id.UserId
    , name : Evergreen.V373.ChannelName.ChannelName
    , description : Evergreen.V373.ChannelDescription.ChannelDescription
    , messages : Evergreen.V373.IdArray.IdArray Evergreen.V373.Id.ChannelMessageId (Evergreen.V373.Message.Message Evergreen.V373.Id.ChannelMessageId (Evergreen.V373.Id.Id Evergreen.V373.Id.UserId))
    , status : ChannelStatus
    , lastTypedAt : SeqDict.SeqDict (Evergreen.V373.Id.Id Evergreen.V373.Id.UserId) (Evergreen.V373.Thread.LastTypedAt Evergreen.V373.Id.ChannelMessageId)
    , threads : SeqDict.SeqDict (Evergreen.V373.Id.Id Evergreen.V373.Id.ChannelMessageId) Evergreen.V373.Thread.BackendThread
    , dateDividerDrawings : SeqDict.SeqDict Date.Date (Evergreen.V373.Drawing.Drawing (Evergreen.V373.Id.Id Evergreen.V373.Id.UserId))
    , games : SeqDict.SeqDict (Evergreen.V373.Id.Id Evergreen.V373.Id.ChannelMessageId) Evergreen.V373.Game.BackendGameData
    }


type alias BackendGuild =
    { createdAt : Effect.Time.Posix
    , createdBy : Evergreen.V373.Id.Id Evergreen.V373.Id.UserId
    , name : Evergreen.V373.GuildName.GuildName
    , icon : Maybe Evergreen.V373.FileStatus.FileHash
    , channels : SeqDict.SeqDict (Evergreen.V373.Id.Id Evergreen.V373.Id.ChannelId) BackendChannel
    , membersAndOwner :
        Evergreen.V373.MembersAndOwner.MembersAndOwner
            (Evergreen.V373.Id.Id Evergreen.V373.Id.UserId)
            { joinedAt : Effect.Time.Posix
            }
    , invites :
        SeqDict.SeqDict
            (Evergreen.V373.SecretId.SecretId Evergreen.V373.Id.InviteLinkId)
            { createdAt : Effect.Time.Posix
            , createdBy : Evergreen.V373.Id.Id Evergreen.V373.Id.UserId
            }
    }


type alias DeletedBackendGuild =
    { guild : BackendGuild
    , deletedAt : Effect.Time.Posix
    }


type alias DiscordBackendChannel =
    { name : Evergreen.V373.ChannelName.ChannelName
    , description : Evergreen.V373.ChannelDescription.ChannelDescription
    , isForum : Bool
    , messages : Evergreen.V373.IdArray.IdArray Evergreen.V373.Id.ChannelMessageId (Evergreen.V373.Message.Message Evergreen.V373.Id.ChannelMessageId (Evergreen.V373.Discord.Id Evergreen.V373.Discord.UserId))
    , status : ChannelStatus
    , lastTypedAt : SeqDict.SeqDict (Evergreen.V373.Discord.Id Evergreen.V373.Discord.UserId) (Evergreen.V373.Thread.LastTypedAt Evergreen.V373.Id.ChannelMessageId)
    , linkedMessageIds : Evergreen.V373.OneToOne.OneToOne (Evergreen.V373.Discord.Id Evergreen.V373.Discord.MessageId) (Evergreen.V373.Id.Id Evergreen.V373.Id.ChannelMessageId)
    , threads : SeqDict.SeqDict (Evergreen.V373.Id.Id Evergreen.V373.Id.ChannelMessageId) Evergreen.V373.Thread.DiscordBackendThread
    , dateDividerDrawings : SeqDict.SeqDict Date.Date (Evergreen.V373.Drawing.Drawing (Evergreen.V373.Discord.Id Evergreen.V373.Discord.UserId))
    , permissionOverwrites : List Evergreen.V373.Discord.Overwrite
    }


type alias DiscordBackendGuild =
    { name : Evergreen.V373.GuildName.GuildName
    , icon : Maybe Evergreen.V373.FileStatus.FileHash
    , channels : SeqDict.SeqDict (Evergreen.V373.Discord.Id Evergreen.V373.Discord.ChannelId) DiscordBackendChannel
    , membersAndOwner :
        Evergreen.V373.MembersAndOwner.MembersAndOwner
            (Evergreen.V373.Discord.Id Evergreen.V373.Discord.UserId)
            { joinedAt : Maybe Effect.Time.Posix
            , roles : SeqSet.SeqSet (Evergreen.V373.Discord.Id Evergreen.V373.Discord.RoleId)
            }
    , stickers : SeqSet.SeqSet (Evergreen.V373.Id.Id Evergreen.V373.Id.StickerId)
    , customEmojis : SeqSet.SeqSet (Evergreen.V373.Id.Id Evergreen.V373.Id.CustomEmojiId)
    , roles : SeqDict.SeqDict (Evergreen.V373.Discord.Id Evergreen.V373.Discord.RoleId) DiscordRole
    }


type alias DiscordThreadReload =
    { threadId : Evergreen.V373.Discord.Id Evergreen.V373.Discord.ChannelId
    , messages : List Evergreen.V373.Discord.Message
    }


type alias DiscordChannelReload =
    { messages : List Evergreen.V373.Discord.Message
    , threads : List DiscordThreadReload
    }
