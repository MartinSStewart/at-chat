...
JS-side reactions (publish offer, pull answer, request-pull-tracks) are
generated by `mockVoiceChatPorts` below: each time the Elm frontend
sends a message to the `voice_chat_to_js` port the mock looks at it
and pushes the matching reply through `voice_chat_from_js`, so the test
only needs to drive the `admin.click` / `user.click` flow.
...
sfuHandshakeTest config =
    startTest
        ...
        startTime
        config
        [ -- mockVoiceChatPorts (defined further down in this file) is the
          -- JS-side stand-in: every time the Elm frontend pushes a message
          -- to voice_chat_to_js, the mock chooses an appropriate FromJs
          -- reply (publish offer / pull answer / request-pull-tracks) and
          -- feeds it back through voice_chat_from_js. That replaces the
