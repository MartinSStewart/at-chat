...
import Html.Events
import Icons
import Id exposing (AnyGuildOrDmIdNoThread(..), ChannelId, ChannelMessageId, DiscordGuildOrDmIdNoThread(..), GuildId, GuildOrDmId, GuildOrDmIdNoThread(..), Id, ThreadMessageId, ThreadRoute(..), ThreadRouteWithMessage(..), UserId)
import Json.Decode
import List.Extra
...
discordGuildNewMessageCount :
    ...
    -> Int
discordGuildNewMessageCount currentUser guildId guild =
    SeqDict.foldl
        (\channelId channel count ->
            channelNewMessageCount (DiscordGuildOrDmId (DiscordGuildOrDmId_Guild guildId channelId)) currentUser channel + count
        )
        0
        guild.channels
...
discordGuildHasNotifications :
    ...
    -> DiscordFrontendGuild
    -> ChannelNotificationType
discordGuildHasNotifications currentUser guildId guild =
    --if SeqSet.member guildId currentUser.notifyOnAllMessages then
    --    case guildNewMessageCount currentUser guildId guild |> OneOrGreater.fromInt of
    --        Just count ->
    --            NewMessageForUser count
    ...
    case SeqDict.get guildId currentUser.discordDirectMentions of
        ...
        Nothing ->
            case discordGuildNewMessageCount currentUser guildId guild |> OneOrGreater.fromInt of
                Just count ->
                    NewMessage count
                    ...
                    (\( guildId, guild ) ->
                        elLinkButton
                            ...
                            (GuildIcon.view
                                localUser.userAgent
                                (case route of
                                    DiscordGuildRoute a _ ->
                                        if a == guildId then
                                            GuildIcon.IsSelected

                                        else
                                            discordGuildHasNotifications localUser.user guildId guild
                                                |> GuildIcon.Normal

                                    _ ->
                                        discordGuildHasNotifications localUser.user guildId guild |> GuildIcon.Normal
                                )
                                guild
                            )
                    )
...
discordMemberLabel isMobile localUser currentUserId userId =
    rowLinkButton
        ...
        (DiscordDmRoute
            (DiscordDmChannelId.fromUserIds currentUserId userId)
            (NoThreadWithFriends Nothing HideMembersTab)
        )
        [ Ui.spacing 8
        , Ui.paddingXY 0 4
...
scrollToBottomDecoder : AnyGuildOrDmIdNoThread -> ThreadRoute -> ScrollPosition -> Json.Decode.Decoder FrontendMsg
    ...
    Ui.column
        ...
        [ channelHeader
            ...
            (case guildOrDmIdNoThread of
                ...
                NormalGuildOrDmId (GuildOrDmId_Guild _ _) ->
                    Ui.row
        ...
        , Ui.column
            ...
            , MessageInput.view
                ...
                (case guildOrDmIdNoThread of
                    ...
                    NormalGuildOrDmId (GuildOrDmId_Dm _) ->
                        "Write a message in this thread"
...
discordChannelColumnNotMobile :
    ...
    -> GuildChannelNameHover
    -> Element FrontendMsg
discordChannelColumnNotMobile localUser guildId guild channelRoute channelNameHover =
    discordChannelColumn False localUser guildId guild channelRoute channelNameHover True
...
discordChannelColumn :
    ...
    -> Bool
    -> Element FrontendMsg
discordChannelColumn isMobile localUser guildId guild channelRoute channelNameHover canScroll2 =
    let
        guildName : String
        guildName =
            GuildName.toString guild.name
        ...
        directMentions =
            SeqDict.get guildId localUser.user.discordDirectMentions
    in
    channelColumnContainer
        [ Ui.el [ MyUi.hoverText guildName ] (Ui.text guildName)
        , elLinkButton
            (Dom.id "guild_inviteLinkCreatorRoute")
            (DiscordGuildRoute guildId DiscordChannel_GuildSettingsRoute)
            [ Ui.width Ui.shrink
            , Ui.Font.color MyUi.font2
            , Ui.width (Ui.px 40)
            , Ui.alignRight
...
discordChannelColumnThreads :
    ...
    -> Element FrontendMsg
discordChannelColumnThreads isMobile channelRoute directMentions localUser guildId channelId channel threads =
    Ui.column
        []
        (SeqDict.toList threads
            |> List.indexedMap
                (\index ( threadMessageIndex, thread ) ->
                    ...
                    Ui.row
                        ...
                        , Ui.attrIf
                            (not isMobile)
                            (Ui.Events.onMouseEnter (MouseEnteredDiscordChannelName guildId channelId threadRoute))
                        , Ui.attrIf
                            (not isMobile)
                            (Ui.Events.onMouseLeave (MouseExitedDiscordChannelName guildId channelId threadRoute))
                        , Ui.clipWithEllipsis
                        , Ui.height (Ui.px channelHeaderHeight)
                        , MyUi.hoverText name
                        , Ui.contentCenterY
                        ...
                        [ elLinkButton
                            ...
                            (DiscordGuildRoute guildId (DiscordChannel_ChannelRoute channelId (ViewThreadWithFriends threadMessageIndex Nothing HideMembersTab)))
                            [ Ui.height Ui.fill
                            , Ui.contentCenterY
                            , Ui.paddingWith
                                { left = 28
                            ...
                            , Ui.el
                                [ (if isSelected && not isMobile then
                                    ...
                                    channelOrThreadHasNotifications
                                        directMentions
                                        (SeqSet.member guildId localUser.user.discordNotifyOnAllMessages)
                                        channelId
                                        (ViewThread threadMessageIndex)
                                        (SeqDict.get
            ...
            , Ui.el
                [ (if isSelected && not isMobile then
                    ...
                    channelOrThreadHasNotifications
                        ...
                        channelId
                        NoThread
                        (SeqDict.get (NormalGuildOrDmId (GuildOrDmId_Guild guildId channelId)) localUser.user.lastViewed)
                        channel
                  )
...
discordChannelColumnRow :
    ...
    -> DiscordFrontendChannel
    -> Element FrontendMsg
discordChannelColumnRow isMobile channelNameHover directMentions channelRoute localUser guildId channelId channel =
    let
        isSelected : Bool
        isSelected =
            case channelRoute of
        ...
        isHover : Bool
        isHover =
            channelNameHover == DiscordGuildChannelNameHover guildId channelId NoThread
    in
    Ui.row
        [ Ui.attrIf isSelected (Ui.background (Ui.rgba 255 255 255 0.15))
        , Ui.attrIf
            (not isMobile)
            (Ui.Events.onMouseEnter (MouseEnteredDiscordChannelName guildId channelId NoThread))
        , Ui.attrIf
            (not isMobile)
            (Ui.Events.onMouseLeave (MouseExitedDiscordChannelName guildId channelId NoThread))
        , Ui.clipWithEllipsis
        , Ui.height (Ui.px channelHeaderHeight)
        , MyUi.hoverText (ChannelName.toString channel.name)
        , Ui.contentCenterY
        ...
        [ elLinkButton
            ...
            (DiscordGuildRoute guildId (DiscordChannel_ChannelRoute channelId (NoThreadWithFriends Nothing HideMembersTab)))
            [ Ui.height Ui.fill
            , Ui.contentCenterY
            , Ui.paddingWith
                { left = 26
            ...
            , Ui.el
                [ (if isSelected && not isMobile then
                    ...
                    channelOrThreadHasNotifications
                        ...
                        channelId
                        NoThread
                        (SeqDict.get (DiscordGuildOrDmId (DiscordGuildOrDmId_Guild guildId channelId)) localUser.user.lastViewed)
                        channel
                  )
                    |> GuildIcon.notificationView localUser.userAgent 0 -3 MyUi.background2
                , Ui.width (Ui.px 20)
        ...
        , if isHover then
            elLinkButton
                ...
                (DiscordGuildRoute guildId (DiscordChannel_EditChannelRoute channelId))
                [ Ui.alignRight
                , Ui.width (Ui.px 26)
                , Ui.contentCenterY
                , Ui.height Ui.fill
