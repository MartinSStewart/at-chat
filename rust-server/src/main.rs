...
pub struct Notification<D> {
    ...
    /// Base64 of the message the sender encrypted, when the server has no readable body to
    /// send. Not part of the declarative notification spec: the service worker reads it,
    /// decrypts it with the conversation's key and shows that in place of `body`.
    #[serde(skip_serializing_if = "Option::is_none")]
