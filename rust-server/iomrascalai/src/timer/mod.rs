...
impl Timer {
    ...
    pub fn new(config: Arc<Config>) -> Timer {
        Timer {
            ...
            byo_time_left: 0,
            config: config,
            current_budget: Duration::from_millis(0),
            main_time_left: 0,
    ...
    pub fn ran_out_of_time(&self, win_ratio: f32) -> bool {
        let fastplay_budget = (1.0 / self.config.time_control.fastplay_budget).floor() as u32;
        let budget5 = if fastplay_budget == 0 {
            self.current_budget
        } else {
            self.current_budget / fastplay_budget
        };
    ...
    fn adjust_time(&mut self) {
        let time_elapsed = self.elapsed().as_millis() as i64;
