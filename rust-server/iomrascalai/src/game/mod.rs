...
impl Game {
    ...
    /// Wrap a pre-constructed `Board` into a fresh `Game`. Intended for
    /// callers that build the position themselves (e.g. from a board state
    /// received over the wire) rather than replaying moves from scratch.
