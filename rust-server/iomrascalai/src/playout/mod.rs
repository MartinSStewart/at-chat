...
use crate::patterns::SmallPatternMatcher;
...
impl Playout {
    ...
    pub fn run(&self, board: &mut Board, initial_move: Option<&Move>, rng: &mut SmallRng) -> PlayoutResult {
    ...
    fn heuristic_set(&self, played_moves: &Vec<Move>, board: &Board, rng: &mut SmallRng) -> Vec<Coord> {
    ...
    fn select_move(&self, board: &Board, heuristic_set: Vec<Coord>, rng: &mut SmallRng) -> Move {
    ...
    fn atari_move(&self, color: Color, board: &Board, rng: &mut SmallRng) -> Option<Move> {
        ...
        match in_danger.next() {
            Some(chain) => {
                ...
                if solutions.len() > 0 { //if we can actually save it
                    let random = rng.random_range(0..solutions.len());
    ...
    fn random_move(&self, color: Color, board: &Board, rng: &mut SmallRng) -> Move {
        ...
        if let Some(first) = playable_move {
            ...
            loop {
                let r = first + rng.random_range(0..(vacant.len() - first + include_pass));
    ...
    fn check_for_ladders(&self, rng: &mut SmallRng) -> bool {
        rng.random_range(0f32..1f32) <= self.config.playout.ladder_check
    }

    fn check_for_atari(&self, rng: &mut SmallRng) -> bool {
        rng.random_range(0f32..1f32) <= self.config.playout.atari_check
    }

    fn use_patterns(&self, rng: &mut SmallRng) -> bool {
        rng.random_range(0f32..1f32) <= self.config.playout.pattern_probability
    }

    fn play_in_middle_of_eye(&self, rng: &mut SmallRng) -> bool {
        rng.random_range(0f32..1f32) <= self.config.playout.play_in_middle_of_eye
    }
